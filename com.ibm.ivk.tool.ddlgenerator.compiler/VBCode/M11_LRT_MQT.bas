    ...
    With g_pools.descriptors(i)
      If .supportLrt Then
        Print #fileNo, addTab(2); "SET v_stmntTxt = 'CALL "; g_schemaNameCtoDbAdmin; "' || "; _
                              "CAST(RIGHT('00' || RTRIM(CAST(ID AS CHAR(2))),2) || '"; CStr(.id); "' AS CHAR(3)) || '."; unqualProcNameMqtSync; _
                              "(?,?)';"
        Print #fileNo,
...
' ### ENDIF IVK ###
  ...
  If acmEntityType = eactClass Then
    With g_classes.descriptors(acmEntityIndex)
      ...
      useDivOidWhereClause = (.aggHeadClassIndex = g_classIndexGenericCode) And Not isPsTagged
      ...
      orMappingRootEntityIndex = .orMappingSuperClassIndex
      useValueCompression = .useValueCompression
...
' ### ENDIF IVK ###
      ...
      isAggHead = .isAggHead And Not forGen And Not forNl
' ### IF IVK ###
      tableIsPsTagged = .isPsTagged And (usePsTagInNlTextTables Or Not forNl)
      objSupportsPsDpFilter = .isPsTagged
' ### ENDIF IVK ###
      useMqtToImplementLrtForEntity = useMqtToImplementLrt And .useMqtToImplementLrt
' ### IF IVK ###
      condenseData = .condenseData
      expandExpressionsInFtoView = .expandExpressionsInFtoView
      
      supportPartitionByClassId = supportRangePartitioningByClassId And Not forNl And .subClassIdStrSeparatePartition.numMaps > 0
      
      hasVirtualAttrs = Not forNl And ((forGen And .hasExpBasedVirtualAttrInGenInclSubClasses) Or (Not forGen And .hasExpBasedVirtualAttrInNonGenInclSubClasses))
      
      isDivTagged = (.navPathToDiv.relRefIndex > 0) And Not (.classIndex = g_classIndexProductStructure)
      If isDivTagged Then
        With g_relationships.descriptors(.navPathToDiv.relRefIndex)
          If .navPathToDiv.navDirectionToClass = etLeft Then
...
' ### ENDIF IVK ###
      ...
      tabSpaceData = .tabSpaceData
      tabSpaceIndexData = .tabSpaceIndexData
      tabSpaceLong = .tabSpaceLong
      tabSpaceIndexLong = .tabSpaceIndexLong
      tabSpaceIndex = .tabSpaceIndex
  ...
  ElseIf acmEntityType = eactRelationship Then
    With g_relationships.descriptors(acmEntityIndex)
      ...
      orMappingRootEntityIndex = .relIndex
      useValueCompression = .useValueCompression
      isCommonToOrgs = .isCommonToOrgs
      isCommonToPools = .isCommonToPools
' ### IF IVK ###
...
' ### IF IVK ###
      ...
      isAllowedCountriesRel = .isAllowedCountries
      isDisallowedCountriesRel = .isDisallowedCountries
      
      If .isDisallowedCountries <> ernmNone Or .isAllowedCountries <> ernmNone Then
        acClassIndex = IIf(.isDisallowedCountries = ernmLeft Or .isAllowedCountries = ernmLeft, .rightEntityIndex, .leftEntityIndex)
        With g_classes.descriptors(acClassIndex)
          acOoParClassIndex = .orMappingSuperClassIndex
          acFkColName = genSurrogateKeyName(ddlType, .shortName)
        End With
      End If
      
      If .isAllowedCountries <> ernmNone Then
        qualCountryListFuncName = genQualFuncName(.sectionIndex, udfnAllowedCountry2Str0, ddlType, thisOrgIndex, thisPoolIndex)
        acColName = g_anAllowedCountries
        acColLength = gc_allowedCountriesMaxLength
      ElseIf .isDisallowedCountries <> ernmNone Then
        qualCountryListFuncName = genQualFuncName(.sectionIndex, udfnDisallowedCountry2Str0, ddlType, thisOrgIndex, thisPoolIndex)
      ...
      isDivTagged = False
      If Not forNl Then
        If .leftIsDivision Then
          isDivTagged = True
          fkAttrToDiv = genSurrogateKeyName(ddlType, .rlShortRelName)
        ElseIf .rightIsDivision Then
          isDivTagged = True
          fkAttrToDiv = genSurrogateKeyName(ddlType, .lrShortRelName)
        End If
      End If
  ...
  If hasVirtualAttrs Then
    ...
    For k = 1 To tabColumns.numDescriptors
      With tabColumns.descriptors(k)
        If .columnCategory And eacVirtual Then
          numVirtualAttrs = numVirtualAttrs + 1
          If .isInstantiated Then numVirtualAttrsInstantiated = numVirtualAttrsInstantiated + 1
        End If
      End With
    Next k
      ...
      With tabColumns.descriptors(k)
        If .columnCategory And eacVirtual Then
          If Not printedHeader Then
            ...
            printedHeader = True
          End If
          genVarDecl fileNoMqt, "v_" & .acmAttributeName, getDbDatatypeByDomainIndex(.dbDomainIndex), "NULL"
        End If
      End With
    Next k
      ...
      With tabColumns.descriptors(k)
        If .columnCategory And eacVirtual Then
          If Not printedHeader Then
            ...
            printedHeader = True
          End If
          Print #fileNoMqt, addTab(1); "SET "; "v_"; .acmAttributeName; " = "; transformAttrName(.columnName, eavtDomain, .dbDomainIndex, transformation, ddlType, , , , True, .acmAttributeIndex, edomValueVirtual, , , True); ";"
        End If
      End With
    Next k
    ...
    For k = 1 To tabColumns.numDescriptors
      With tabColumns.descriptors(k)
        If (.columnCategory And eacVirtual) And .isInstantiated Then
          If Not firstAttr Then
            Print #fileNoMqt, ","
          End If
          firstAttr = False
          Print #fileNoMqt, addTab(2); "PUB."; .columnName; " = v_"; .acmAttributeName;
        End If
      End With
    Next k
    ...
    For k = 1 To tabColumns.numDescriptors
      With tabColumns.descriptors(k)
        If .columnCategory And eacVirtual Then
          ...
          setAttributeMapping transformation, 2 + numVirtualAttrs, .columnName, "v_" & .acmAttributeName
        End If
      End With
    Next k
  End If
...
' ### IF IVK ###
  If hasVirtualAttrs Then
    ...
    For k = 1 To tabColumns.numDescriptors
      With tabColumns.descriptors(k)
        If (.columnCategory And eacVirtual) <> 0 And .isInstantiated Then
          numVirtualAttrsInstantiated = numVirtualAttrsInstantiated + 1
          setAttributeMapping transformation, 2 + numVirtualAttrsInstantiated, .columnName, gc_newRecordName & "." & UCase(.columnName)
        End If
      End With
    Next k
  End If
...
' ### IF IVK ###
      If ((.columnCategory And eacOid) <> eacOid) And ((.columnCategory And eacExpression) <> eacExpression) Then
' ### ELSE IVK ###
...
' ### ENDIF IVK ###
        ...
        If False Then
          ...
          Print #fileNoMqt, addTab(6); "(COALESCE(MQT."; .columnName; ", PUB."; .columnName; ") IS NOT NULL AND (MQT."; .columnName; " IS NULL OR PUB."; .columnName; " IS NULL OR MQT."; .columnName; " <> PUB."; .columnName; "))"
        Else
          Print #fileNoMqt, addTab(6); "MQT."; .columnName; " <> PUB."; .columnName
        End If
        firstCol = False
      End If
    End With
...
' ### ENDIF IVK ###
        ...
        If False Then
          ' todo: consider null-values here
          Print #fileNoMqt, addTab(6); "(COALESCE(MQT."; .columnName; ", PRIV."; .columnName; ") IS NOT NULL AND (MQT."; .columnName; " IS NULL OR PRIV."; .columnName; " IS NULL OR MQT."; .columnName; " <> PRIV."; .columnName; "))"
        Else
          Print #fileNoMqt, addTab(6); "MQT."; .columnName; " <> PRIV."; .columnName
        End If
        firstCol = False
      End If
    End With
