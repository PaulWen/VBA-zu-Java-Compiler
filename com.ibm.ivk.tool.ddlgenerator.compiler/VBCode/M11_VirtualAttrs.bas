...
Sub genVirtAttrSupportForEntity( _
  ...
  If acmEntityType = eactClass Then
    With g_classes.descriptors(acmEntityIndex)
      ...
      hasExpBasedVirtualAttrs = Not forNl And ((forGen And .hasExpBasedVirtualAttrInGenInclSubClasses) Or (Not forGen And .hasExpBasedVirtualAttrInNonGenInclSubClasses))
      hasRelBasedVirtualAttrs = Not forNl And ((forGen And .hasRelBasedVirtualAttrInGenInclSubClasses) Or (Not forGen And .hasRelBasedVirtualAttrInNonGenInclSubClasses))
      hasVirtualAttrs = hasExpBasedVirtualAttrs Or hasRelBasedVirtualAttrs
      attrRefsInclSubClasses = .attrRefsInclSubClassesWithRepeat
    End With
  ...
  If hasRelBasedVirtualAttrs Then
    ...
    For i = 1 To attrRefsInclSubClasses.numDescriptors
      If attrRefsInclSubClasses.descriptors(i).refType = eadrtAttribute And attrRefsInclSubClasses.descriptors(i).refIndex > 0 Then
        With g_attributes.descriptors(attrRefsInclSubClasses.descriptors(i).refIndex)
          If (.isTimeVarying = forGen) And .isVirtual Then
            virtAttrlist = virtAttrlist & ", " & .dbColName(ddlType)
            ...
            relIndex = .virtuallyMapsTo.relIndex
            relNavDirection = .virtuallyMapsTo.navDirection
            sourceClassIndex = .acmEntityIndex
            sourceOrParClassIndex = g_classes.descriptors(.acmEntityIndex).orMappingSuperClassIndex
            targetOrParClassIndex = g_classes.descriptors(.virtuallyMapsTo.targetClassIndex).orMappingSuperClassIndex
          End If
        End With
      End If
    Next i
    ...
    With g_relationships.descriptors(relIndex)
      fkAttrName = IIf(relNavDirection = etLeft, .rightFkColName(ddlType), .leftFkColName(ddlType))
    End With
        ...
        With g_attributes.descriptors(attrRefsInclSubClasses.descriptors(i).refIndex)
          If (.isTimeVarying = forGen) And .isVirtual Then
            If colonMissing Then
              Print #fileNo, ","
            End If
            Print #fileNo, addTab(2); .dbColName(ddlType);
            colonMissing = True
          End If
        End With
      End If
        ...
        With g_attributes.descriptors(attrRefsInclSubClasses.descriptors(i).refIndex)
          If (.isTimeVarying = forGen) And .isVirtual Then
            If colonMissing Then
              Print #fileNo, ","
            End If
            Print #fileNo, addTab(3); "COALESCE(S."; genAttrName(.virtuallyMapsTo.mapTo & IIf(.valueType = eavtEnum, gc_enumAttrNameSuffix, ""), ddlType); ", T."; genAttrName(.attributeName & IIf(.valueType = eavtEnum, gc_enumAttrNameSuffix, ""), ddlType); ")";
            colonMissing = True
          End If
        End With
      End If
    ...
    With g_classes.descriptors(sourceClassIndex)
      If Not .hasOwnTable Then
        Print #fileNo, addTab(3); "AND"
        Print #fileNo, addTab(2); "T."; g_anCid; " IN ("; .subclassIdStrListNonAbstract; ")"
      End If
    End With
      ...
      With tabColumns.descriptors(k)
        If .columnCategory And eacVirtual Then
          With g_domains.descriptors(.dbDomainIndex)
            If (.dataType = etChar Or .dataType = etClob Or .dataType = etLongVarchar Or .dataType = etVarchar) Then
          ...
          If virtualColumnSyncCommitCount > 0 Then
            ...
            genProcSectionHeader fileNo, "update virtual column """ & UCase(.columnName) & """ in ""commit window"" of table""" & qualTabName & """", 2
            Print #fileNo, addTab(2); "UPDATE"
            Print #fileNo, addTab(2); "("
            Print #fileNo, addTab(3); "SELECT"
            Print #fileNo, addTab(4); "T."; g_anOid; ","
            Print #fileNo, addTab(4); "T."; .columnName; ","
            ...
            Print #fileNo, addTab(3); "V."; .columnName; " = "; transformAttrName(.columnName, eavtDomain, .dbDomainIndex, transformation, ddlType, , , , True, .acmAttributeIndex, edomValueVirtual)
            Print #fileNo, addTab(2); "WHERE"
            Print #fileNo, addTab(3); "COALESCE(V."; .columnName; ","; invalidColValue; ") <> COALESCE("; _
                                      transformAttrName(.columnName, eavtDomain, .dbDomainIndex, transformation, ddlType, , , , True, .acmAttributeIndex, edomValueVirtual); ","; invalidColValue; ")"
            Print #fileNo, addTab(4); "AND"
          ...
          Else
            genProcSectionHeader fileNo, "update virtual column """ & UCase(.columnName) & """ in table""" & qualTabName & """"
            Print #fileNo, addTab(1); "UPDATE"
            Print #fileNo, addTab(2); qualTabName; " T"
            Print #fileNo, addTab(1); "SET"
            Print #fileNo, addTab(2); "T."; .columnName; " = "; transformAttrName(.columnName, eavtDomain, .dbDomainIndex, transformation, ddlType, , , , True, .acmAttributeIndex, edomValueVirtual)
            Print #fileNo, addTab(1); "WHERE"
            Print #fileNo, addTab(2); "COALESCE(T."; .columnName; ","; invalidColValue; ") <> COALESCE("; _
                                      transformAttrName(.columnName, eavtDomain, .dbDomainIndex, transformation, ddlType, , , , True, .acmAttributeIndex, edomValueVirtual); ","; invalidColValue; ")"
            Print #fileNo, addTab(1); ";"
