...
' ### ENDIF IVK ###
  ...
  If acmEntityType = eactClass Then
    With g_classes.descriptors(acmEntityIndex)
      aggHeadClassIndex = .aggHeadClassIndex
      useMqtToImplementLrt = .useMqtToImplementLrt
      isAggHead = .isAggHead
      implicitelyGenChangeComment = .implicitelyGenChangeComment
      hasNl = .hasNlAttrsInNonGenInclSubClasses
      
' ### IF IVK ###
      hasNoIdentity = .hasNoIdentity
      enforceLrtChangeComment = .enforceLrtChangeComment
      isPsTagged = .isPsTagged
      useClassIdFilter = useClassIdFilter And (UCase(.className) <> UCase(clnGenericAspect)) ' list of class-IDs too long
' ### ENDIF IVK ###
    End With
...
' ### IF IVK ###
          If (Not strArrayIsNull(.mapsToChangeLogAttributes)) And .isNl And (forGen Or hasNoIdentity) = .isTimeVarying Then
' ### ELSE IVK ###
'         If (Not strArrayIsNull(.mapsToChangeLogAttributes)) And .isNl And forGen = .isTimeVarying Then
' ### ENDIF IVK ###
            For j = LBound(.mapsToChangeLogAttributes) To UBound(.mapsToChangeLogAttributes)
              columnName = genAttrName(.attributeName, ddlType)
              
              columnMaxLengthSrc = getMaxDbAttributeLengthByNameAndEntityIndex(columnName, acmEntityType, acmEntityIndex)
              columnMaxLengthDst = getMaxDbAttributeLengthByNameAndEntityIndex(.mapsToChangeLogAttributes(j), eactClass, g_classIndexChangeLog)
    
              genProcSectionHeader fileNo, "add NL-Text-Column """ & columnName & """ / """ & UCase(.mapsToChangeLogAttributes(j)) & """ to changelog entries", indent, Not addNewLine
              ...
              Print #fileNo, addTab(indent + 1); UCase(.mapsToChangeLogAttributes(j)); ","
              Print #fileNo, addTab(indent + 1); "PRIO,"
' ### IF IVK ###
              If usePsTagInNlTextTables Then
                Print #fileNo, addTab(indent + 1); g_anPsOid; ","
    ...
    If Not forGen Then
      For i = 1 To relRefs.numRefs
        With g_relationships.descriptors(relRefs.refs(i).refIndex)
          If Not arrayIsNull(.refersToClAttribute) And .reusedRelIndex <= 0 Then
            If relRefs.refs(i).refType = etRight Then
              referredClassIndex = .leftEntityIndex
              referringClassIndex = .rightEntityIndex
              relFkAttrName = .leftFkColName(ddlType)
            Else
              referredClassIndex = .rightEntityIndex
              referringClassIndex = .leftEntityIndex
              relFkAttrName = .rightFkColName(ddlType)
            End If
            ...
            If referringClassIndex > 0 Then
              ...
              Dim l As Integer
              For l = 1 To .reusingRelIndexes.numIndexes
                With g_relationships.descriptors(.reusingRelIndexes.indexes(l))
                  addClassIdToList entityIdStrListForRel, IIf(relRefs.refs(i).refType = etRight, .rightEntityIndex, .leftEntityIndex), True
                End With
              Next l
            End If
            
            For j = LBound(.refersToClAttribute) To UBound(.refersToClAttribute)
              attrBasePrio = .refersToClAttribute(j).prio
              attrIndex = getAttributeIndexByName(g_classes.descriptors(referredClassIndex).sectionName, .refersToClAttribute(j).mapTo)
  
              If attrIndex > 0 Then
                With g_attributes.descriptors(attrIndex)
                  If .isNl Then
                    columnName = genAttrName(.attributeName, ddlType)
                    columnMaxLengthSrc = getMaxDbAttributeLengthByNameAndEntityIndex(columnName, .cType, .acmEntityIndex)
                    ...
                    With g_classes.descriptors(g_classes.descriptors(referredClassIndex).orMappingSuperClassIndex)
                      refColName = genSurrogateKeyName(ddlType, .shortName)
                    End With
                    With g_classes.descriptors(g_classes.descriptors(referringClassIndex).orMappingSuperClassIndex)
                      tabVarRef = UCase(.shortName)
                    End With
                    
                    If .isTimeVarying Then
                      With g_classes.descriptors(g_classes.descriptors(referredClassIndex).orMappingSuperClassIndex)
                        tabVarNl = UCase(.shortName) & "GNL"
                        tabVarGen = UCase(.shortName) & "G"
                      End With
                      ...
                      genProcSectionHeader fileNo, "add NL-Text-Column """ & _
                          ...
                          """ (" & UCase(.attributeName) & " @ " & g_classes.descriptors(referredClassIndex).className & " (TV)) to changelog entries", indent
                    ...
                    Else
                      ...
                      genProcSectionHeader fileNo, "add NL-Text-Column """ & _
                          UCase(g_relationships.descriptors(relRefs.refs(i).refIndex).refersToClAttribute(j).mapFrom) & _
                          """ (" & UCase(.attributeName) & " @ " & g_classes.descriptors(referredClassIndex).className & ") to changelog entries", indent
...
' ### ENDIF IVK ###
  ...
  For i = 1 To tabColumns.numDescriptors
    With tabColumns.descriptors(i)
      If (.columnCategory And eacRegular) Then
        genVarDecl fileNo, "v_last_" & .acmAttributeName, getDbDatatypeByDomainIndex(.dbDomainIndex), "NULL", indent + 1
      End If
    End With
  Next i
  ...
  For i = 1 To tabColumns.numDescriptors
    With tabColumns.descriptors(i)
      If (.columnCategory And eacRegular) Then
        Print #fileNo, addTab(indent + 3); .columnName; " AS c_"; .acmAttributeName; ","
      End If
    End With
  Next i
...
' ### ENDIF IVK ###
  For i = 1 To tabColumns.numDescriptors
    With tabColumns.descriptors(i)
      If (.columnCategory And eacRegular) Then
        setAttributeMapping transformation, tIndex, .columnName, "v_last_" & .acmAttributeName
        tIndex = tIndex + 1
      End If
    End With
  Next i
...
' ### ENDIF IVK ###
  ...
  For i = 1 To tabColumns.numDescriptors
    With tabColumns.descriptors(i)
      If (.columnCategory And eacRegular) Then
        Print #fileNo, addTab(indent + 3); "SET v_last_"; .acmAttributeName; " = c_"; .acmAttributeName; ";"
      End If
    End With
  Next i
...
' ### ENDIF IVK ###
  ...
  For i = 1 To tabColumns.numDescriptors
    With tabColumns.descriptors(i)
      If (.columnCategory And eacRegular) Then
        Print #fileNo, addTab(indent + 3); "IF c_"; .acmAttributeName; " IS NOT NULL THEN"
        Print #fileNo, addTab(indent + 4); "SET v_last_"; .acmAttributeName; " = c_"; .acmAttributeName; ";"
...
' ### IF IVK ###
      isGenericAspect = UCase(.className) = UCase(clnGenericAspect) And Not forGen And Not forNl
' ### ENDIF IVK ###
      
      If forNl Then
        entityName = genNlObjName(.className, , forGen)
        entityShortName = genNlObjShortName(.shortName, , forGen, True)
...
' ### ENDIF IVK ###
        isLogChange = False
        isGenForming = False
        attrRefs = .nlAttrRefsInclSubclasses
        hasOwnTable = True
      Else
...
' ### IF IVK ###
        hasNoIdentity = .hasNoIdentity
' ### ENDIF IVK ###
        isLogChange = .logLastChange
        isGenForming = .isGenForming
        If forGen Then
          hasNlAttrs = .hasNlAttrsInGenInclSubClasses
        Else
          hasNlAttrs = .hasNlAttrsInNonGenInclSubClasses
        End If
        attrRefs = .attrRefs
      
' ### IF IVK ###
        hasPriceAssignmentSubClass = .hasPriceAssignmentSubClass
        isSubjectToPreisDurchschuss = .isSubjectToPreisDurchschuss
        enforceLrtChangeComment = .enforceLrtChangeComment
        ...
        End If
' ### ENDIF IVK ###
        isAggHead = (.aggHeadClassIndex > 0) And (.orMappingSuperClassIndex = .aggHeadClassIndex)
      End If
      entityIsTransactional = .isUserTransactional
      entityIdStr = .classIdStr
      entityIdStrList = getSubClassIdStrListByClassIndex(.classIndex)
      dbAcmEntityType = gc_acmEntityTypeKeyClass
      aggHeadClassIndex = .aggHeadClassIndex
      hasSubClass = .hasSubClass
      nlAttrRefs = .nlAttrRefsInclSubclasses
      hasOwnTable = .hasOwnTable
' ### IF IVK ###
      isPsTagged = .isPsTagged
      condenseData = .condenseData
      isNationalizable = .isNationalizable And Not forNl
      hasIsNationalInclSubClasses = .hasIsNationalInclSubClasses And Not forNl
' ### ENDIF IVK ###
    End With
  ElseIf acmEntityType = eactRelationship Then
    With g_relationships.descriptors(acmEntityIndex)
      sectionIndex = .sectionIndex
      If forNl Then
        entityName = genNlObjName(.relName, , forGen)
        entityShortName = genNlObjShortName(.shortName, , forGen, True)
      ...
      Else
        ...
        isLogChange = .logLastChange
        hasNlAttrs = .nlAttrRefs.numDescriptors > 0
        attrRefs = .attrRefs
' ### IF IVK ###
        priceAssignmentHasNlAttrs = hasNlAttrs
' ### ENDIF IVK ###
...
' ### IF IVK ###
      ...
      hasIsNationalInclSubClasses = .hasIsNationalInclSubClasses
      isSubjectToPreisDurchschuss = .isSubjectToPreisDurchschuss
' ### ENDIF IVK ###
    End With
  Else
...
' ### IF IVK ###
  If isSubjectToPreisDurchschuss Then
    With g_classes.descriptors(g_classIndexGenericAspect)
      Dim i As Integer
      For i = 1 To UBound(.subclassIndexesRecursive)
        With g_classes.descriptors(.subclassIndexesRecursive(i))
          If Not .isAbstract And .isPriceAssignment Then
            priceAssignmentSubClassIdList = priceAssignmentSubClassIdList & IIf(priceAssignmentSubClassIdList <> "", ",", "") & "'" & .classIdStr & "'"
            priceAssignmentHasNlAttrs = priceAssignmentHasNlAttrs Or .hasNlAttrsInNonGenInclSubClasses
          End If
        End With
      Next i
    End With
...
' ### ENDIF IVK ###
      While Not hasColumnToFilter And (attrIndex <= tabColumns.numDescriptors)
        With tabColumns.descriptors(attrIndex)
' ### IF IVK ###
          If (.columnCategory And eacRegular) Or (.columnCategory And eacFkOid) Or ((clMode = eclLrt) And (.columnCategory And eacSetProdMeta)) Then
' ### ELSE IVK ###
'         If (.columnCategory And eacRegular) Or (.columnCategory And eacFkOid) Then
' ### ENDIF IVK ###
            If .acmAttributeIndex > 0 Then
              With g_attributes.descriptors(.acmAttributeIndex)
                hasColumnToFilter = hasColumnToFilter Or (g_classes.descriptors(.acmEntityIndex).superClassIndex > 0)
              End With
            End If
            numAttrs = numAttrs + 1
          End If
      ...
      While Not hasColumnToFilter And (attrIndex <= nlAttrRefs.numDescriptors)
        If nlAttrRefs.descriptors(attrIndex).refType = eadrtAttribute Then
          With g_attributes.descriptors(nlAttrRefs.descriptors(attrIndex).refIndex)
' ### IF IVK ###
            If .isNl And (forGen Or hasNoIdentity) = .isTimeVarying Then
' ### ELSE IVK ###
...
' ### ENDIF IVK ###
      ...
      While thisAttributeIndex <= tabColumns.numDescriptors
        With tabColumns.descriptors(thisAttributeIndex)
          If isClAttrCat(.columnCategory, clMode = eclLrt) Then
            ignoreForChangelog = False
            
            If .acmFkRelIndex > 0 Then
              If g_relationships.descriptors(.acmFkRelIndex).ignoreForChangelog Then
                ignoreForChangelog = True
              End If
            End If
            
' ### IF IVK ###
            If clMode = eclLrt And .columnName = g_anIsBlockedPrice Then
              ignoreForChangelog = True
            End If
...
' ### IF IVK ###
              ...
              If clMode = eclLrt And .columnName = g_anStatus Then
...
'                setManActConditional = Not isPrimaryOrg And hasIsNationalInclSubClasses And isSubjectToPreisDurchschuss
              ...
              If .acmAttributeIndex > 0 Then
                If g_attributes.descriptors(.acmAttributeIndex).isExpression Then
                  alternativeColumnName = genSurrogateKeyName(ddlType, g_attributes.descriptors(.acmAttributeIndex).shortName & "EXP")
                End If
              End If
                                       
              genGenChangeLogRecordForCTEDdl "updates on '" & .columnName & "'" & " (" & thisAttributeIndex & ")", _
                                       fileNo, stringsPerType, valuesStringForCTE, caseUpdateStringForCTE, splitVar, qualTabNameExpr, _
                                       ddlType, .columnName, alternativeColumnName, g_domains.descriptors(.dbDomainIndex).dataType, clMode, .columnCategory, _
                                       columnTargetValue, .isNullable
   ...
   End If
      ...
      While thisAttributeIndex <= nlAttrRefs.numDescriptors
        If nlAttrRefs.descriptors(thisAttributeIndex).refType = eadrtAttribute Then
          With g_attributes.descriptors(nlAttrRefs.descriptors(thisAttributeIndex).refIndex)
' ### IF IVK ###
            If .isNl And (forGen Or hasNoIdentity) = .isTimeVarying Then
...
' ### ENDIF IVK ###
              ...
              genGenChangeLogRecordForCTEDdl "updates on NL-column '" & columnName & "'" & " (" & thisAttributeIndex & ")", _
                                       ...
                                       g_domains.descriptors(.domainIndex).dataType, clMode, , , True
              
' ### IF IVK ###
              
              If .isNationalizable Then
                Dim natColumnName As String
                natColumnName = genAttrName(.attributeName, ddlType, , , , , True)
                ...
                natColumnName = genAttrName(.attributeName & gc_anSuffixNatActivated, ddlType)
                  
                genGenChangeLogRecordForCTEDdl _
                   "updates on NL-column '" & natColumnName & "'" & " (" & thisAttributeIndex & ")", _
                  fileNo, stringsPerType, valuesStringForCTE, caseUpdateStringForCTE, False, qualTabNameExpr, ddlType, natColumnName, , _
                  g_domains.descriptors(.domainIndex).dataType, clMode, , , False
...
' ### ENDIF IVK ###
  ...
  If acmEntityType = eactClass Then
    With g_classes.descriptors(acmEntityIndex)
      ...
      hasOwnTable = .hasOwnTable
      entityIdStr = .classIdStr
      relRefs = .relRefsRecursive
      isGenForming = .isGenForming
      attrMapping = .clMapAttrsInclSubclasses
      dbAcmEntityType = gc_acmEntityTypeKeyClass
      isAggHead = (.aggHeadClassIndex > 0) And (.orMappingSuperClassIndex = .aggHeadClassIndex) And Not forGen And Not forNl
' ### IF IVK ###
      isPsTagged = .isPsTagged
      hasNoIdentity = .hasNoIdentity
      navPathToDiv = .navPathToDiv
      condenseData = .condenseData
' ### ENDIF IVK ###
    End With
  ElseIf acmEntityType = eactRelationship Then
...
' ### ENDIF IVK ###
      ...
      reuseRelIndex = IIf(reuseRelationships And .reusedRelIndex > 0, .reusedRelIndex, acmEntityIndex)
      With g_relationships.descriptors(reuseRelIndex)
        With g_classes.descriptors(.leftEntityIndex)
          relLeftClassIdStr = .classIdStr
          relLeftFk = genAttrName(conOid, ddlType, .shortName)
        End With
        With g_classes.descriptors(.rightEntityIndex)
          relRightClassIdStr = .classIdStr
          relRightFk = genAttrName(conOid, ddlType, .shortName)
        End With
      End With
    End With
  Else
    ...
    For i = 1 To tabColumns.numDescriptors
      With tabColumns.descriptors(i)
        If isClAttrCat(.columnCategory, clMode = eclLrt) Then
          ...
          attrTypeId = g_domains.descriptors(.dbDomainIndex).dataType
          Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 2); "_t,"
          
          Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 2); "_o,"
          Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 2); "_n,"
          
          If attrTypeMapsToClColType(attrTypeId, clValueTypeTimeStamp) Then
            Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 3); "_To,"
            Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 3); "_Tn,"
          End If
          
          If attrTypeMapsToClColType(attrTypeId, clValueTypeDate) Then
            Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 3); "_Dto,"
            Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 3); "_Dtn,"
          End If
          
          If attrTypeMapsToClColType(attrTypeId, clValueTypeInteger) Then
            Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 3); "_Io,"
            Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 3); "_In,"
          End If
          
          If attrTypeMapsToClColType(attrTypeId, clValueTypeBigInteger) Then
            Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 3); "_BIo,"
            Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 3); "_BIn,"
          End If
          
          If attrTypeMapsToClColType(attrTypeId, clValueTypeDecimal) Then
            Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 3); "_Do,"
            Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 3); "_Dn,"
          End If
          
          If attrTypeMapsToClColType(attrTypeId, clValueTypeBoolean) Then
            Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 3); "_Bo,"
            Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 3); "_Bn,"
          End If
        End If
      End With
    Next i
...
' ### ENDIF IVK ###
  ...
  If acmEntityType = eactClass Then
    With g_classes.descriptors(acmEntityIndex)
      ...
      sectionIndex = .sectionIndex
      entityIdStr = .classIdStr
      hasNlAttributes = IIf(forGen, .hasNlAttrsInGenInclSubClasses, .hasNlAttrsInNonGenInclSubClasses)
      attrMapping = .clMapAttrsInclSubclasses
' ### IF IVK ###
      isPsTagged = .isPsTagged
      hasNoIdentity = .hasNoIdentity
      isNationalizable = .isNationalizable
      aggHeadClassIndex = .aggHeadClassIndex
      ignoreForChangelog = .ignoreForChangelog
      condenseData = .condenseData
      allowedCountriesRelIndex = .allowedCountriesRelIndex
      disAllowedCountriesRelIndex = .disAllowedCountriesRelIndex
      allowedCountriesListRelIndex = .allowedCountriesListRelIndex
      disAllowedCountriesListRelIndex = .disAllowedCountriesListRelIndex
      useMqtToImplementLrtForEntity = useMqtToImplementLrt And .useMqtToImplementLrt
      
      checkAggHeadForAttrs = (.aggHeadClassIndex > 0) And ((.orMappingSuperClassIndex <> .aggHeadClassIndex) Or forGen)
      isAggHead = (.aggHeadClassIndex > 0) And (.orMappingSuperClassIndex = .aggHeadClassIndex)
' ### ENDIF IVK ###
    End With
  ElseIf acmEntityType = eactRelationship Then
    With g_relationships.descriptors(acmEntityIndex)
      ...
      relRefs.numRefs = 0
      isGenForming = False
      hasNlAttributes = .nlAttrRefs.numDescriptors > 0
      ...
      aggHeadClassIndex = .aggHeadClassIndex
      checkAggHeadForAttrs = (.aggHeadClassIndex > 0)
      ignoreForChangelog = .ignoreForChangelog
' ### IF IVK ###
      isPsTagged = .isPsTagged
      hasNoIdentity = False
...
' ### ENDIF IVK ###
  ...
  If checkAggHeadForAttrs Then
    With g_classes.descriptors(aggHeadClassIndex)
      attrMappingAh = .clMapAttrsInclSubclasses
      relRefsAh = .relRefsRecursive
      aggHeadSupportMqt = useMqtToImplementLrt And .useMqtToImplementLrt
    End With
  End If
...
' ### IF IVK ###
  ...
  If foundPropertyOid Then
    ...
    With g_classes.descriptors(IIf(propertyTargetClassIndex > 0, propertyTargetClassIndex, IIf(propertyTargetClassIndexAh > 0, propertyTargetClassIndexAh, IIf(foundPropertyOidInAggHead, aggHeadClassIndex, acmEntityIndex))))
      If clMode = eclLrt Then
        qualObjNamePropertyGen = genQualViewNameByClassIndex(.classIndex, ddlType, thisOrgIndex, srcPoolIndex, True, True, .useMqtToImplementLrt)
      Else
        qualObjNamePropertyGen = genQualTabNameByClassIndex(.classIndex, ddlType, thisOrgIndex, srcPoolIndex, True)
      End If
    End With
...
' ### IF IVK ###
  ...
  If allowedCountriesRelIndex > 0 Then
    With g_relationships.descriptors(allowedCountriesRelIndex)
      qualCountryListFuncName = _
        genQualFuncName( _
          .sectionIndex, udfnAllowedCountry2Str0 & qualCountryListFuncNameSuffix, ddlType, thisOrgIndex, srcPoolIndex _
        )
    End With
  ...
  If disAllowedCountriesRelIndex > 0 Then
    With g_relationships.descriptors(disAllowedCountriesRelIndex)
      qualCountryListFuncName = _
        genQualFuncName( _
          .sectionIndex, udfnDisallowedCountry2Str0 & qualCountryListFuncNameSuffix, ddlType, thisOrgIndex, srcPoolIndex _
        )
    End With
...
' ### IF IVK ###
          If (.columnCategory And eacExpression) Then
            ...
            newVal = transformAttrName(.columnName, eavtDomain, .dbDomainIndex, transformationExpr, ddlType, , , , , .acmAttributeIndex, edomValueExpression, , , , .columnCategory)
            ...
            oldVal = transformAttrName(.columnName, eavtDomain, .dbDomainIndex, transformationExpr, ddlType, , , , , .acmAttributeIndex, edomValueExpression, , , , .columnCategory)
...
' ### ENDIF IVK ###
            ...
            If attrTypeId = etBigInt Or _
              ...
              Print #fileNo, addTab(2); "RTRIM(CAST("; tupVarTgt; "."; .columnName; " AS CHAR(30))),"
              Print #fileNo, addTab(2); "RTRIM(CAST("; tupVarSrc; "."; .columnName; " AS CHAR(30))),"
            Else
              Print #fileNo, addTab(2); "CAST("; tupVarTgt; "."; .columnName; " AS VARCHAR(4000)),"
              Print #fileNo, addTab(2); "CAST("; tupVarSrc; "."; .columnName; " AS VARCHAR(4000)),"
            End If
          
            ' oldValueTimestamp / newValueTimestamp
            If attrTypeMapsToClColType(attrTypeId, clValueTypeTimeStamp) Then
              Print #fileNo, addTab(2); tupVarTgt; "."; .columnName; ","
              Print #fileNo, addTab(2); tupVarSrc; "."; .columnName; ","
            End If
            ...
            If attrTypeMapsToClColType(attrTypeId, clValueTypeDecimal) Then
              Print #fileNo, addTab(2); "CAST("; tupVarTgt; "."; .columnName; " AS DECIMAL(31,10)),"
              Print #fileNo, addTab(2); "CAST("; tupVarSrc; "."; .columnName; " AS DECIMAL(31,10)),"
            End If
...
' ### ENDIF IVK ###
    ...
    For i = 1 To tabColumns.numDescriptors
      With tabColumns.descriptors(i)
' ### IF IVK ###
        If isClAttrCat(.columnCategory, clMode = eclLrt) And ((.columnCategory And eacNationalEntityMeta) = 0) Then
' ### ELSE IVK ###
'       If isClAttrCat(.columnCategory, clMode = eclLrt) Then
' ### ENDIF IVK ###
          Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 2); "_t,"
          ...
          If attrTypeMapsToClColType(g_domains.descriptors(.dbDomainIndex).dataType, clValueTypeInteger) Then
            Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 3); "_Io,"
            Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 3); "_In,"
          End If
        End If
    ...
    For i = 1 To tabColumns.numDescriptors
      With tabColumns.descriptors(i)
        If isClAttrCat(.columnCategory, clMode = eclLrt) Then
          ...
          If attrTypeMapsToClColType(attrTypeId, clValueTypeTimeStamp) Then
            Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 3); "_To,"
            newValueString = Left(.columnName, gc_dbMaxAttributeNameLength - 3) & "_Tn"
          End If
          
          If attrTypeMapsToClColType(attrTypeId, clValueTypeDate) Then
            Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 3); "_Dto,"
            newValueString = Left(.columnName, gc_dbMaxAttributeNameLength - 3) & "_Dtn"
          End If
          
          If attrTypeMapsToClColType(attrTypeId, clValueTypeInteger) Then
            Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 3); "_Io,"
            newValueString = Left(.columnName, gc_dbMaxAttributeNameLength - 3) & "_In"
          End If
          
          If attrTypeMapsToClColType(attrTypeId, clValueTypeBigInteger) Then
            Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 3); "_BIo,"
            newValueString = Left(.columnName, gc_dbMaxAttributeNameLength - 3) & "_BIn"
          End If
          
          If attrTypeMapsToClColType(attrTypeId, clValueTypeDecimal) Then
            Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 3); "_Do,"
            newValueString = Left(.columnName, gc_dbMaxAttributeNameLength - 3) & "_Dn"
          End If
          
          If attrTypeMapsToClColType(attrTypeId, clValueTypeBoolean) Then
            Print #fileNo, addTab(1); Left(.columnName, gc_dbMaxAttributeNameLength - 3); "_Bo,"
            newValueString = Left(.columnName, gc_dbMaxAttributeNameLength - 3) & "_Bn"
          End If
          If .columnName <> "LASTUPDATETIMESTAMP" And newValueString <> "" Then
...
' ### ENDIF IVK ###
            ...
            If attrTypeId = etBigInt Or _
               ...
               attrTypeId = etTimestamp Or _
               attrTypeId = etBoolean Then
              Print #fileNo, addTab(2); "RTRIM(CAST("; tupVarTgt; "."; .columnName; " AS VARCHAR(50))),"
              Print #fileNo, addTab(2); "RTRIM(CAST("; tupVarSrc; "."; .columnName; " AS VARCHAR(50))),"
            End If
            ...
            If attrTypeMapsToClColType(attrTypeId, clValueTypeTimeStamp) Then
              Print #fileNo, addTab(2); tupVarTgt; "."; .columnName; ","
              newValueColumn = tupVarSrc & "." & .columnName
            End If
            ...
            If .columnName <> "LASTUPDATETIMESTAMP" And newValueColumn <> "" Then
