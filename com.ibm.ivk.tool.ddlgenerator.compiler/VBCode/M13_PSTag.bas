...
Private Sub genPsTagSupportDdlForEntity( _
  ...
  If acmEntityType = eactClass Then
    With g_classes.descriptors(acmEntityIndex)
      ...
      entityInsertable = (.updateMode And eupmInsert)
      entityUpdatable = (.updateMode And eupmUpdate)
      entityDeletable = (.updateMode And eupmDelete)
      isCtoAliasCreated = .isCtoAliasCreated
      noAlias = .noAlias
      ignorePsRegVarOnInsertDelete = .ignPsRegVarOnInsDel
      useSurrogateKey = .useSurrogateKey
      isChangeLog = (UCase(.className) = UCase(clnChangeLog))
      expandExpressionsInFtoView = .expandExpressionsInFtoView
      isNotAcmRelated = .notAcmRelated
      
      If psTagOptional And .hasSubClass Then
        Dim i As Integer
        For i = 1 To UBound(.subclassIndexesRecursive)
          With g_classes.descriptors(.subclassIndexesRecursive(i))
            If .isPsTagged And Not .psTagOptional Then
              subclassIdListMandatoryPsTag = subclassIdListMandatoryPsTag & IIf(subclassIdListMandatoryPsTag <> "", ",", "") & "'" & .classIdStr & "'"
            End If
          End With
        Next i
      End If
  ...
  If ddlType = edtPdm And thisPoolIndex > 0 Then
    With g_pools.descriptors(thisPoolIndex)
      ...
      poolSupportPsTaggingView = .supportViewsForPsTag
      poolSupportPsTaggingTrigger = .supportTriggerForPsTag
    End With
  End If
  ...
  If generatePsTaggingTrigger And poolSupportPsTaggingTrigger And useSurrogateKey Then
    ...
    For i = 1 To 3
      ...
      Else
        ...
        If isChangeLog Then
          With g_classes.descriptors(g_classIndexChangeLogStatus)
            If Not ((.specificToPool > 0 And .specificToPool <> g_pools.descriptors(thisPoolIndex).id) Or (.specificToOrgId > 0 And .specificToOrgId <> g_orgs.descriptors(thisOrgIndex).id)) Then
