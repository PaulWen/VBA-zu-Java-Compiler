...
Private Sub genXmlExportViewSupportDdlForNlTable( _
  ...
  With attrRefs
    ...
    For i = 1 To .numDescriptors
      If onlyThisAttribute = -1 Or (onlyThisAttribute = .descriptors(i).refIndex) Then
        With g_attributes.descriptors(.descriptors(i).refIndex)
          If IIf(inclTvAttrs, .isTimeVarying, False) Or IIf(inclNonTvAttrs, Not .isTimeVarying, False) Then
            Print #fileNo, addTab(indent + 5); "XMLELEMENT (NAME """; UCase(.attributeName); """, L."; UCase(.attributeName); "),"
          End If
        End With
      End If
    Next i
...
Private Sub genXmlExportViewDdlForEntity( _
  ...
  If acmEntityType = eactClass Then
    With g_classes.descriptors(acmEntityIndex)
      orMappingEntityIndex = .orMappingSuperClassIndex
      orEntityShortName = g_classes.descriptors(.orMappingSuperClassIndex).shortName
      ...
      hasGenTab = .isGenForming And Not .hasNoIdentity
      ...
      qualViewName = genQualViewNameByClassIndex(.classIndex, ddlType, thisOrgIndex, thisPoolIndex, False, , , , , gc_xmlObjNameSuffix)
      qualViewNameLdm = genQualViewNameByClassIndex(.classIndex, edtLdm, thisOrgIndex, thisPoolIndex, False, , , , , gc_xmlObjNameSuffix)
    End With
  ElseIf acmEntityType = eactRelationship Then
    With g_relationships.descriptors(acmEntityIndex)
      orMappingEntityIndex = .relIndex
      sectionName = .sectionName
      sectionShortName = .sectionShortName
      sectionIndex = .sectionIndex
      ...
      ElseIf navToFirstClassToDivDirection = etRight Then
        ...
        fkAttrToClass = genSurrogateKeyName(ddlType, .lrShortRelName)
      ...
      qualTabName = genQualTabNameByRelIndex(.relIndex, ddlType, thisOrgIndex, thisPoolIndex)
      qualViewName = genQualViewNameByRelIndex(.relIndex, ddlType, thisOrgIndex, thisPoolIndex, False, , , , gc_xmlObjNameSuffix)
      qualViewNameLdm = genQualViewNameByRelIndex(.relIndex, edtLdm, thisOrgIndex, thisPoolIndex, False, , , , gc_xmlObjNameSuffix)
    End With
  ElseIf acmEntityType = eactEnum Then
    With g_enums.descriptors(acmEntityIndex)
      orMappingEntityIndex = .enumIndex
      sectionName = .sectionName
      sectionShortName = .sectionShortName
      sectionIndex = .sectionIndex
      ...
      qualTabName = genQualTabNameByEnumIndex(.enumIndex, ddlType, thisOrgIndex, thisPoolIndex)
      qualViewName = genQualViewNameByEnumIndex(.enumIndex, ddlType, thisOrgIndex, thisPoolIndex, False, , gc_xmlObjNameSuffix)
      qualViewNameLdm = genQualViewNameByEnumIndex(.enumIndex, edtLdm, thisOrgIndex, thisPoolIndex, False, , gc_xmlObjNameSuffix)
    End With
  End If
  ...
  If generateXmlExportViews Then
    ...
    With g_domains.descriptors(g_domainIndexTemplateFileData)
      setAttributeMapping transformation, 1, , , .sectionName, .domainName
    End With
    With g_domains.descriptors(g_domainIndexBinaryPropertyValue)
      setAttributeMapping transformation, 2, conValue, "XMLELEMENT (NAME """ & g_anValue & """, CAST(NULL AS VARCHAR(1)))", .sectionName, .domainName
      setAttributeMapping transformation, 3, , , .sectionName, .domainName
    End With
    With g_domains.descriptors(g_domainIndexBIBRegistryValue)
      setAttributeMapping transformation, 4, , , .sectionName, .domainName
    End With
    With g_domains.descriptors(g_domainIndexLongText)
      setAttributeMapping transformation, 5, , , .sectionName, .domainName
    End With
    With g_domains.descriptors(g_domainIndexReportFileData)
      setAttributeMapping transformation, 6, , , .sectionName, .domainName
    End With
    ...
    If acmEntityType = eactClass Then
      ...
      With g_classes.descriptors(acmEntityIndex)
        If .isGenForming And Not .hasNoIdentity Then
          ...
          With transformationGen
            If .numNlAttrRefsTv > 0 Then
    ...
    With transformation
      If (acmEntityType = eactEnum) Or (IIf(Not hasGenTab, .numNlAttrRefsTv, 0) + .numNlAttrRefsNonTv > 0) Then
  ...
  If generateXmlExportFuncs Then
    ...
    End With
    With g_domains.descriptors(g_domainIndexBinaryPropertyValue)
      setAttributeMapping transformation, 2, conValue, "T." & g_anValue & "", .sectionName, .domainName
      setAttributeMapping transformation, 3, , , .sectionName, .domainName
    End With
    With g_domains.descriptors(g_domainIndexBIBRegistryValue)
      setAttributeMapping transformation, 4, , , .sectionName, .domainName
    ...
    ' NL-attributes for non-GEN table
    With transformation
      If IIf(Not hasGenTab, .numNlAttrRefsTv, 0) + .numNlAttrRefsNonTv > 0 Then
        qualNlTabName = genQualTabNameByEntityIndex(orMappingEntityIndex, acmEntityType, ddlType, thisOrgIndex, thisPoolIndex, , , , True)
  ...
  With attrRefs
    ...
    For i = 1 To .numDescriptors
      If onlyThisAttribute = -1 Or (onlyThisAttribute = .descriptors(i).refIndex) Then
        With g_attributes.descriptors(.descriptors(i).refIndex)
          If IIf(inclTvAttrs, .isTimeVarying, False) Or IIf(inclNonTvAttrs, Not .isTimeVarying, False) Then
            Print #fileNo, addTab(2); "'"; addTab(indent + 6); "<element name="""; UCase(.attributeName); _
                                      """ type=""standardxml:"; .sectionName; "_"; .domainName; IIf(.isNullable, "_N", ""); """/>' || cr ||"
          End If
        End With
      End If
    Next i
...
Private Sub genXmlExportXsdFuncForEntity( _
  ...
  End With
  With g_domains.descriptors(g_domainIndexBinaryPropertyValue)
    setAttributeMapping transformation, 2, conValue, g_anValue, .sectionName, .domainName
    setAttributeMapping transformation, 3, , , .sectionName, .domainName
  End With
  With g_domains.descriptors(g_domainIndexBIBRegistryValue)
    setAttributeMapping transformation, 4, , , .sectionName, .domainName
  ...
  If acmEntityType = eactClass Then
    ...
    With g_classes.descriptors(acmEntityIndex)
      If .isGenForming And Not .hasNoIdentity Then
        ...
        With transformationGen
          If .numNlAttrRefsTv > 0 Then
            genXmlExportXsdFuncSupportForNlTable .nlAttrRefs, "NlText", fileNo, ddlType, , True, False, 15
          End If
        End With
  ...
  With transformation
    If (acmEntityType = eactEnum) Or (IIf(Not hasGenTab, .numNlAttrRefsTv, 0) + .numNlAttrRefsNonTv > 0) Then
      If acmEntityType = eactEnum Then
        genXmlExportXsdFuncSupportForNlTable .nlAttrRefs, "NlText", fileNo, ddlType, , False, True, , conEnumLabelText, snCommon, "EnumText", False
      Else
        genXmlExportXsdFuncSupportForNlTable .nlAttrRefs, "NlText", fileNo, ddlType, , Not hasGenTab, True
      End If
    End If
    ...
    If acmEntityType = eactEnum Then
      With g_enums.descriptors(acmEntityIndex)
        genSimpleTypesForXML fileNo, transformation, .sectionName, , .idDataType, , val(.maxLength), , , , True, .sectionName & gc_enumAttrNameSuffix
      End With
...
Sub genXmlExportDdlForClass( _
  ...
  With g_classes.descriptors(classIndex)
    If .notAcmRelated Or .isAbstract Or .noXmlExport Then
      Exit Sub
    End If
  End With
...
Sub genXmlExportDdlForRelationship( _
  ...
  With g_relationships.descriptors(thisRelIndex)
    If .notAcmRelated Or .noXmlExport Then
      Exit Sub
    End If
  End With
...
Private Sub genSimpleTypesForXML( _
  ...
  Else
    ...
    For i = 1 To transformation.domainRefs.numRefs Step 1
      With g_domains.descriptors(transformation.domainRefs.refs(i).ref)
        If supportUnicode And .supportUnicode And .maxLength <> "" Then
          printSimpleTypeForXML .dataType, fileNo, .sectionName, .domainName, .minLength, _
            IIf(supportUnicode And .supportUnicode, CInt(.unicodeExpansionFactor * CInt(.maxLength)) & "", .maxLength), .scale, .minValue, .maxValue, , _
            transformation.domainRefs.refs(i).isNullable
        Else
          printSimpleTypeForXML .dataType, fileNo, .sectionName, .domainName, .minLength, _
            .maxLength, .scale, .minValue, .maxValue, , transformation.domainRefs.refs(i).isNullable
        End If
      End With
    Next i
  End If
