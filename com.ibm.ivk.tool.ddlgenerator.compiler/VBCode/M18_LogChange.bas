...
Private Sub genInsertChangeLogBroadcastCall( _
  ...
  With g_classes.descriptors(acmEntityIndex)
    entityName = UCase(.className)
    classId = .classIdStr
    ahClassId = .aggHeadClassIdStr
    If .aggHeadClassIdStr <> "" Then
      ahClassId = .aggHeadClassIdStr
    Else
      ahClassId = .classIdStr
    End If
  End With
...
Private Sub genInsertRelChangeLogBroadcastCall( _
  ...
  With g_relationships.descriptors(acmEntityIndex)
    entityName = UCase(.relName)
    classId = .relIdStr
    leftEntityIndex = .leftEntityIndex
    leftFkColName = .leftFkColName(1)
  End With
...
Private Sub genUpdateChangeLogBroadcastCall( _
  ...
  With g_classes.descriptors(acmEntityIndex)
    If nlText And Not distNlText Then
      entityName = UCase(.className) & "_NL_TEXT"
    Else
      entityName = UCase(.className)
    End If
    shortName = .shortName
  ...
  With colDesc
    ...
    dataType = g_domains.descriptors(.dbDomainIndex).dataType
...
' ### IF IVK ###
      supportTriggerForPsTag = .supportViewsForPsTag
        ...
        If broadcastForDist Then
          ...
          While thisAttributeIndex <= tabColumns.numDescriptors
            With tabColumns.descriptors(thisAttributeIndex)
             ...
             If isClAttrCat(.columnCategory, False) Then
...
Private Sub genLogChangeAutoMaintSupportDdlForEntity( _
  ...
  If acmEntityType = eactClass Then
    With g_classes.descriptors(acmEntityIndex)
      ...
      entityShortName = .shortName
      entityTypeDescr = "ACM-Class"
      logLastChange = .logLastChange
      isCtp = .isCommonToPools
      isUserTransactional = .isUserTransactional
