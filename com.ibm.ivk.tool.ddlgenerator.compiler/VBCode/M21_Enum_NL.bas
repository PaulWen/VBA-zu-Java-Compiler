...
Private Sub readSheet()
  ...
  If numLangsForEnumsNl > 0 Then
    While thisSheet.Cells(thisRow, colI18nId) & "" <> ""
      ...
      With g_enumsNl.descriptors(allocEnumNlDescriptorIndex(g_enumsNl))
...
Sub evalEnumsNl()
  ...
  With g_enumsNl
    For i = 1 To .numDescriptors Step 1
      With .descriptors(i)
        .enumIndex = getEnumIndexByI18nId(.i18nId)
      End With
    Next i
  End With
End Sub
...
Sub genEnumNlAcmMetaCsv( _
  ...
  For i = 1 To g_enumsNl.numDescriptors
    With g_enumsNl.descriptors(i)
      For j = 1 To numLangsForEnumsNl
        If .nl(j) <> "" And .enumIndex > 0 Then
          Print #fileNo, """"; UCase(g_enums.descriptors(.enumIndex).sectionName); ""","; _
                         """"; UCase(g_enums.descriptors(.enumIndex).enumName); ""","; _
