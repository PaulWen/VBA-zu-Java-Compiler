...
Private Sub readSheet()
  ...
  While thisSheet.Cells(thisRow, colDataPool) & "" <> ""
    ...
    With g_pools.descriptors(allocDataPoolIndex(g_pools))
      .id = thisPoolId
      ...
      .supportLrt = getBoolean(thisSheet.Cells(thisRow, colSupportLRT))
      
' ### IF IVK ###
      .supportViewsForPsTag = getBoolean(thisSheet.Cells(thisRow, colSupportViewsForPsTag))
      .supportTriggerForPsTag = getBoolean(thisSheet.Cells(thisRow, colSupportTriggerForPsTag))
      .supportXmlExport = getBoolean(thisSheet.Cells(thisRow, colSupportXmlExport))
' ### ENDIF IVK ###
      .supportUpdates = getBoolean(thisSheet.Cells(thisRow, colSupportUpdates))
      
      .suppressRefIntegrity = getBoolean(thisSheet.Cells(thisRow, colSuppressRefIntegrity))
      .suppressUniqueConstraints = getBoolean(thisSheet.Cells(thisRow, colSuppressUniqueConstraints))
' ### IF IVK ###
      .instantiateExpressions = getBoolean(thisSheet.Cells(thisRow, colInstantiateExpressions))
' ### ENDIF IVK ###
      .commonItemsLocal = getBoolean(thisSheet.Cells(thisRow, colCommonItemsLocal))
      .supportAcm = getBoolean(thisSheet.Cells(thisRow, colSupportAcm))
      
      .isActive = getBoolean(thisSheet.Cells(thisRow, colIsActive))
' ### IF IVK ###
      .isArchive = getBoolean(thisSheet.Cells(thisRow, colIsArchive))
      .isProductive = getBoolean(thisSheet.Cells(thisRow, colIsProductive))
      .supportNationalization = getBoolean(thisSheet.Cells(thisRow, colSupportNationalization))
' ### ENDIF IVK ###
    
      .sequenceCacheSize = getInteger(thisSheet.Cells(thisRow, colSequenceCacheSize), -1)
' ### IF IVK ###
      
      If .isArchive And Not supportArchivePool Then
        .isActive = False
      End If
' ### ENDIF IVK ###
    End With
...
Function poolIsValidForOrg( _
  ...
  Else
    With g_pools.descriptors(thisPoolIndex)
      poolIsValidForOrg = (.specificToOrgId = -1 Or .specificToOrgId = g_orgs.descriptors(thisOrgIndex).id)
    End With
  End If
End Function
...
Sub genDataPoolDdl2( _
  ...
  If ddlType = edtPdm Then
    With g_pools.descriptors(thisPoolIndex)
      If (Not .supportAcm) Or .isArchive Then
        Exit Sub
      End If
    End With
  End If
...
Sub genDataPoolDdl3( _
  ...
  With g_pools.descriptors(thisPoolIndex)
    If (Not .supportAcm) Or .specificToOrgId > 0 Then
      Exit Sub
    End If
...
Sub genSrxUDFsByPool( _
  ...
  With g_classes.descriptors(g_classIndexGenericAspect)
    qualFuncName = genQualFuncName(.sectionIndex, srxTypeStr & "Ctxt_OID", ddlType, thisOrgIndex, thisPoolIndex)
  End With
...
'  Print #fileNo, addTab(1); "RETURN (CASE WHEN v_oidIdList = '' THEN NULL ELSE v_oidIdList END);"
  ...
  With g_classes.descriptors(g_classIndexGenericAspect)
    qualFuncName = genQualFuncName(.sectionIndex, srxTypeStr & "Ctxt_CDE", ddlType, thisOrgIndex, thisPoolIndex)
  End With
...
'  Print #fileNo, addTab(1); "RETURN (CASE WHEN v_codeNumList = '' THEN NULL ELSE v_codeNumList END);"
  ...
  If Not (thisOrgIndex = g_primaryOrgIndex) And (thisPoolIndex = g_workDataPoolIndex) Then
    ...
    With g_classes.descriptors(g_classIndexGenericAspect)
      qualFuncName = genQualFuncName(.sectionIndex, srxTypeStr & "IsAvail", ddlType, thisOrgIndex, thisPoolIndex)
    End With
  ...
  If srxType = estSr0 Then
    With g_classes.descriptors(g_classIndexGenericAspect)
      qualFuncName = genQualFuncName(.sectionIndex, "IsValidForSr0", ddlType, thisOrgIndex, thisPoolIndex)
    End With
