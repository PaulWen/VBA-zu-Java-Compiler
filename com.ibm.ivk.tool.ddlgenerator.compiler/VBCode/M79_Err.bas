...
Private Sub readSheet()
  ...
  While thisSheet.Cells(thisRow, colId) & "" <> ""
    ...
    If getBoolean(thisSheet.Cells(thisRow, colIsActive)) Then
      With g_errs.descriptors(allocErrDescriptorIndex(g_errs))
        .id = Trim(thisSheet.Cells(thisRow, colId))
        .isTechnical = getBoolean(thisSheet.Cells(thisRow, colIsTechnical))
        .sqlStateOffset = getInteger(thisSheet.Cells(thisRow, colSqlState))
        .busErrorMessageNo = Trim(thisSheet.Cells(thisRow, colBusErrorMessageNo))
        .messagePattern = Trim(thisSheet.Cells(thisRow, colMessagePattern))

        .messageExplanation = Trim(thisSheet.Cells(thisRow, colMessageExplanation))
        .conEnumLabelText = Trim(thisSheet.Cells(thisRow, colContext))
      End With
    End If
...
Private Function transformErrMsg( _
  ...
  With g_errs.descriptors(msgIndex)
    result = Replace(.messagePattern, "%1", arg1 & "")
    If .busErrorMessageNo <> "" Then
      result = Replace(result, "%b", .busErrorMessageNo & "")
    End If
  End With
...
Sub genSignalDdl( _
  ...
  For i = 1 To g_errs.numDescriptors
    With g_errs.descriptors(i)
      If UCase(.id) = UCase(id) Then
        Print #fileNo, addTab(indent); "SIGNAL SQLSTATE '"; CStr(getSqlStateByOffset(.sqlStateOffset)); _
...
Sub genErrorCsv( _
  ...
  For i = 1 To g_errs.numDescriptors
    With g_errs.descriptors(i)
      Print #fileNo, CStr(.sqlStateOffset + g_cfgSqlStateStart); ",";
      Print #fileNo, IIf(.busErrorMessageNo <> "", .busErrorMessageNo, ""); ",";
      Print #fileNo, """"; Replace(.messageExplanation, """", """"""); """,";
      Print #fileNo, """"; Replace(Replace(.conEnumLabelText, newline, "\n", , , vbBinaryCompare), """", """"""); """,";
