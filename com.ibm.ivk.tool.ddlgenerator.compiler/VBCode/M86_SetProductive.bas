...
Sub genSetProdSupportSpsForEntity( _
  ...
  If acmEntityType = eactClass Then
    With g_classes.descriptors(acmEntityIndex)
      ...
      aggChildClassIndexes = .aggChildClassIndexes
    End With
  ...
  If navToDivRelRefIndex > 0 Then
    With g_relationships.descriptors(g_classes.descriptors(g_classIndexProductStructure).navPathToDiv.relRefIndex)
      If g_classes.descriptors(g_classIndexProductStructure).navPathToDiv.navDirection = etLeft Then
        psFkAttrToDiv = .leftFkColName(ddlType)
      Else
        psFkAttrToDiv = .rightFkColName(ddlType)
      End If
    End With
  ...
  If acmEntityIndex = g_classIndexExpression Then
    ...
    For i = 1 To UBound(aggChildClassIndexes)
      With g_classes.descriptors(aggChildClassIndexes(i))
        If .classIndex = .orMappingSuperClassIndex Then
          Print #fileNo,
          
          Dim j As Integer
          For j = 1 To IIf(.isGenForming And Not .hasNoIdentity, 2, 1)
            Print #fileNo, addTab(2); "UPDATE"
            Print #fileNo, addTab(3); genQualTabNameByClassIndex(.classIndex, ddlType, thisOrgIndex, srcPoolIndex, (j = 2)); " S"
            Print #fileNo, addTab(2); "SET"
            Print #fileNo, addTab(3); "S."; g_anHasBeenSetProductive; " = 1,"
            If .classIndex = g_classIndexExpression Then
            ...
            Print #fileNo, addTab(5); "1"
            Print #fileNo, addTab(4); "FROM"
            Print #fileNo, addTab(5); genQualTabNameByClassIndex(.classIndex, ddlType, thisOrgIndex, dstPoolIndex, (j = 2)); " T"
            Print #fileNo, addTab(4); "WHERE"
            ...
            Print #fileNo, addTab(2); "UPDATE"
            Print #fileNo, addTab(3); genQualTabNameByClassIndex(.classIndex, ddlType, thisOrgIndex, dstPoolIndex, (j = 2)); " S"
            Print #fileNo, addTab(2); "SET"
            Print #fileNo, addTab(3); "S."; g_anHasBeenSetProductive; " = 1,"
            If .classIndex = g_classIndexExpression Then
              Print #fileNo, addTab(3); "S."; g_anStatus; " = 5,"
...
Sub genSetProdSupportDdlForClass( _
  ...
  With g_classes.descriptors(classIndex)
    genSetProdSupportSpsForEntity .classIndex, eactClass, thisOrgIndex, thisPoolIndex, dstPoolIndex, fileNo, fileNoClView, ddlType, forGen
  
    If IIf(forGen, .hasNlAttrsInGenInclSubClasses, .hasNlAttrsInNonGenInclSubClasses) Then
      genSetProdSupportSpsForEntity .classIndex, eactClass, thisOrgIndex, thisPoolIndex, dstPoolIndex, fileNo, fileNoClView, ddlType, forGen, True
    End If
  End With
End Sub
...
Sub genSetProdSupportDdlForRelationship( _
  ...
  With g_relationships.descriptors(thisRelIndex)
    genSetProdSupportSpsForEntity .relIndex, eactRelationship, thisOrgIndex, thisPoolIndex, dstPoolIndex, fileNo, fileNoClView, ddlType, forGen
  
    If .nlAttrRefs.numDescriptors > 0 Then
      genSetProdSupportSpsForEntity .relIndex, eactRelationship, thisOrgIndex, thisPoolIndex, dstPoolIndex, fileNo, fileNoClView, ddlType, forGen, True
    End If
  End With
End Sub
