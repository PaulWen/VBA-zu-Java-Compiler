    ...
    For thisOrgIndex = 1 To g_orgs.numDescriptors Step 1
      For thisPoolIndex = 1 To g_pools.numDescriptors Step 1
        With g_pools.descriptors(thisPoolIndex)
          If poolIsValidForOrg(thisPoolIndex, thisOrgIndex) And Not .commonItemsLocal And .supportAcm And Not .isArchive Then
  ...
  For i = IIf(implementCodesWithoutDepViaRegDynamic, 1, 2) To IIf(implementCodesWithoutDepViaOidList, 3, 2)
    ...
    For j = 1 To tabColumns.numDescriptors
      If tabColumns.descriptors(j).acmAttributeIndex > 0 Then
        With g_attributes.descriptors(tabColumns.descriptors(j).acmAttributeIndex)
          If .isExpression And (tabColumns.descriptors(j).columnCategory And (eacNational Or eacNationalBool)) = 0 Then
            If Not isFirstLoop Then
              Print #fileNo, addTab(3); "UNION ALL"
            End If
            Print #fileNo, addTab(2); "SELECT DISTINCT A."; genSurrogateKeyName(ddlType, .shortName & "EXP"); " FROM "; qualTabNameGenericAspect; " A WHERE A."; g_anPsOid; " = psOid_in"
            If .isNationalizable Then
              Print #fileNo, addTab(3); "UNION ALL"
              Print #fileNo, addTab(2); "SELECT DISTINCT A."; genSurrogateKeyName(ddlType, .shortName & "EXP", , , , True); " FROM "; qualTabNameGenericAspect; " A WHERE A."; g_anPsOid; " = psOid_in"
            End If
            isFirstLoop = False
          End If
        End With
