    ...
    For thisOrgIndex = 1 To g_orgs.numDescriptors Step 1
      ...
      For thisPoolIndex = 1 To g_pools.numDescriptors Step 1
        With g_pools.descriptors(thisPoolIndex)
          If poolIsValidForOrg(thisPoolIndex, thisOrgIndex) And .commonItemsLocal Then
...
Sub genDbUtilitiesDdlByPool( _
  ...
  With g_domains.descriptors(g_domainIndexCountryIdList)
    countryIdListLength = .maxLength
  End With
  ...
  If generateUpdatableCheckInUpdateTrigger And Not forPool And Not generateFwkTest Then
    ...
    For k = 1 To tabColumns.numDescriptors
      With tabColumns.descriptors(k)
        Print #fileNo, addTab(3); .columnName; " AS c_"; .acmAttributeName; IIf(k = tabColumns.numDescriptors, "", ",")
      End With
    Next k
    ...
    For k = 1 To tabColumns.numDescriptors
      With tabColumns.descriptors(k)
        If k > 1 Then
          Print #fileNo, addTab(6); "AND"
        End If
        If .columnName = "SIZEFACTOR" Then
          Print #fileNo, addTab(5); gc_newRecordName; "."; .columnName; " > (0.9 * c_"; .acmAttributeName; ")"
          Print #fileNo, addTab(6); "AND"
          Print #fileNo, addTab(5); gc_newRecordName; "."; .columnName; " < (1.1 * c_"; .acmAttributeName; ")"
        Else
          Print #fileNo, addTab(5); gc_newRecordName; "."; .columnName; " = c_"; .acmAttributeName
        End If
      End With
    Next k
...
Sub genDdlForRegStaticSstUpdate( _
  ...
  If useStaticSql Then
    ...
    For k = 1 To tabColumns.numDescriptors
      With tabColumns.descriptors(k)
        Print #fileNo, addTab(indent + 1); .columnName; IIf(k = tabColumns.numDescriptors, "", ",")
      End With
    Next k
    ...
    For k = 1 To tabColumns.numDescriptors
      With tabColumns.descriptors(k)
        Print #fileNo, addTab(indent + 1); transformAttrName(.columnName, eavtDomain, .dbDomainIndex, transformation, ddlType, , , , True, .acmAttributeIndex, edomList); IIf(k < tabColumns.numDescriptors, ",", "")
      End With
    Next k
