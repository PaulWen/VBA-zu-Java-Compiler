  ...
  If noPartitioningInDataPools <> "" Then
    With g_pools
      ...
      For i = 1 To .numDescriptors
        With .descriptors(i)
          If includedInList(noPartitioningInDataPools, .id) Then
            If andOrFlag Then
              Print #fileNo, addTab(5); "AND"
            End If
            genProcSectionHeader fileNo, "no data partitioning in datapool " & CStr(.id), 5, True
            Print #fileNo, addTab(4); "COALESCE(P."; g_anPoolTypeId; ",-1) <> "; CStr(.id)
            andOrFlag = True
          End If
        End With
      Next i
