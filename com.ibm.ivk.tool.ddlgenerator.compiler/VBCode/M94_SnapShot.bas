...
Private Sub genRecreateSnapshotTabDdl( _
      ...
      Print #fileNo, addTab(3); "'"; _
                                Replace( _
                                  genTransformedAttrDeclByDomain( _
                                    .acmAttributeName, attributeShortName, eavtDomain, .dbDomainIndex, transformation, _
                                    acmEntityType, acmEntityIndex, IIf(.isNullable, "", "NOT NULL") & _
  ...
  If ddlType = edtPdm Then
    With g_classes.descriptors(acmEntityIndex)
      If .tabSpaceData <> "" Then
        Print #fileNo, addTab(2); "'IN "; genTablespaceNameByIndex(.tabSpaceIndexData); " ' || CHR(10) ||"
      End If
      If .tabSpaceLong <> "" Then
        Print #fileNo, addTab(2); "'LONG IN "; genTablespaceNameByIndex(.tabSpaceIndexLong); " ' || CHR(10) ||"
      End If
      If .tabSpaceIndex <> "" Then
        Print #fileNo, addTab(2); "'INDEX IN "; genTablespaceNameByIndex(.tabSpaceIndexIndex); " ' || CHR(10) ||"
      End If
      If .useValueCompression Then
...
Private Sub genDbSnapshotDdlAdmin( _
  ...
  For j = 1 To g_snapshotTypes.numDescriptors
    With g_snapshotTypes.descriptors(j)
      classIndexSnapshot = .classIndex
      
      If tabSpaceNameSnapshot = "" Then
        tabSpaceNameSnapshot = g_classes.descriptors(classIndexSnapshot).tabSpaceData
      End If
      
      genRecreateSnapshotTabDdl fileNo, j, eactClass, classIndexSnapshot, tempTabNameCrTabStmnt, ddlType, .viewName
    End With
  Next j
  ...
  For i = 1 To g_classes.numDescriptors
    With g_classes.descriptors(i)
      If .tabSpaceData <> tabSpaceNameSnapshot Or .sectionName = snDbMonitor Then
        GoTo NextI
      End If
                
      genRecreateSnapshotTabDdl fileNo, j, eactClass, .classIndex, tempTabNameCrTabStmnt, ddlType
      j = j + 1
    End With
NextI:
  Next i
  ...
  For i = 1 To g_relationships.numDescriptors
    With g_relationships.descriptors(i)
      ...
      genRecreateSnapshotTabDdl fileNo, j, eactRelationship, .relIndex, tempTabNameCrTabStmnt, ddlType
      j = j + 1
    End With
NextII:
  Next i
  ...
  For thisTabSpaceIndex = 1 To g_tableSpaces.numDescriptors
    With g_tableSpaces.descriptors(thisTabSpaceIndex)
      If .isMonitor Then
        tsNameList = tsNameList & IIf(tsNameList = "", "", ", ") & "'" & UCase(.tableSpaceName) & "'"
      End If
    End With
  ...
  For thisTabSpaceIndex = 1 To g_tableSpaces.numDescriptors
    With g_tableSpaces.descriptors(thisTabSpaceIndex)
      If .isMonitor Then
        genProcSectionHeader fileNo, "determine statements for drop and create of tablespace """ & .tableSpaceName & """"
        Print #fileNo, addTab(1); "SET v_stmntTxtDropTs = 'DROP TABLESPACE "; UCase(.tableSpaceName); "';"
        Print #fileNo,
        ...
        Print #fileNo, addTab(2); "UPPER(T.TBSPACE) = '"; UCase(.tableSpaceName); "'"
        Print #fileNo, addTab(1); ";"
        Print #fileNo,
        Print #fileNo, addTab(1); "SET v_tsPageSize       = COALESCE(v_tsPageSize       , "; IIf(.pageSize <> "", .pageSize, "4096"); ");"
        Print #fileNo, addTab(1); "SET v_tsType           = COALESCE(v_tsType           , '"; IIf(.category = tscDms, "D", "S"); "');"
        Print #fileNo, addTab(1); "SET v_tsBufferpoolName = COALESCE(v_tsBufferpoolName , '"; genBufferPoolNameByIndex(.bufferPoolIndex); "');"
        Print #fileNo, addTab(1); "SET v_tsExtentSize     = COALESCE(v_tsExtentSize     , "; .extentSize; ");"
        Print #fileNo, addTab(1); "SET v_tsPrefetchSize   = COALESCE(v_tsPrefetchSize   , "; .prefetchSize; ");"
        Print #fileNo,
        Print #fileNo, addTab(1); "SET v_stmntTxt ="
        Print #fileNo, addTab(2); "'CREATE "; IIf(.type <> "", UCase(.type) & " ", ""); "TABLESPACE ' || CHR(10) ||"
        
        Print #fileNo, addTab(3); "'"; UCase(.tableSpaceName); " ' || CHR(10) ||"
        ...
        Print #fileNo, addTab(2); "WHERE"
        Print #fileNo, addTab(3); "C.TABLESPACE_NAME = '"; UCase(.tableSpaceName); "'"
        Print #fileNo, addTab(2); "ORDER BY"
        ...
        If .autoResize Then
          ...
          If .increasePercent > 0 Then
            Print #fileNo, addTab(4); "'INCREASESIZE "; CStr(.increasePercent); " PERCENT ' || CHR(10) ||"
          ElseIf .increaseAbsolute <> "" Then
            Print #fileNo, addTab(4); "'INCREASESIZE "; .increaseAbsolute; " ' || CHR(10) ||"
          End If
          If .maxSize <> "" Then
            Print #fileNo, addTab(4); "'MAXSIZE "; .maxSize; " ' || CHR(10) ||"
          End If
        ...
        numContainerRefs = .containerRefs.numDescriptors
        If .category = tscSms Then
          With .containerRefs
            For j = 1 To numContainerRefs
              Print #fileNo, addTab(3); "'"; genContainerNameByIndex(.descriptors(j)); "'"; IIf(j = numContainerRefs, "", ","); " ' || CHR(10 ||"
            Next j
        ...
        Else
          With .containerRefs
            For j = 1 To numContainerRefs
              With g_containers.descriptors(.descriptors(j))
                Print #fileNo, addTab(3); "'  "; IIf(.type = cntFile, "FILE", "DEVICE"); " "; _
                      "''"; genContainerNameByIndex(.containerIndex); "''"; " "; _
                      CStr(.size); IIf(j = numContainerRefs, "", ","); " ' || chr(10) ||"
              End With
            Next j
            Print #fileNo, addTab(2); "') ' || CHR(10) ||"
          End With
          ...
          If .autoResize Then
            ...
            If .increasePercent > 0 Then
              Print #fileNo, addTab(3); "'INCREASESIZE "; CStr(.increasePercent); " PERCENT ' || CHR(10) ||"
            ElseIf .increaseAbsolute <> "" Then
              Print #fileNo, addTab(3); "'INCREASESIZE "; .increaseAbsolute; " ' || CHR(10) ||"
            End If
            
            If .maxSize <> "" Then
              Print #fileNo, addTab(3); "'MAXSIZE "; .maxSize; " ' || CHR(10) ||"
            End If
          End If
        End If
        ...
        Print #fileNo, addTab(2); "'BUFFERPOOL "; genBufferPoolNameByIndex(.bufferPoolIndex); " ' || CHR(10) ||"
        
        If ddlType = edtPdm Then
          Print #fileNo, addTab(2); "'"; IIf(Not .useFileSystemCaching, "NO ", ""); "FILE SYSTEM CACHING ' || CHR(10) ||"
          ...
          Print #fileNo, addTab(2); "'DROPPED TABLE RECOVERY "; IIf(.supportDroppedTableRecovery, "ON", "OFF"); " ' || CHR(10) ||"
        End If
        Print #fileNo, addTab(2); "''"
        Print #fileNo, addTab(1); ";"
        ...
        Print #fileNo, addTab(2); "'CALL "; qualProcNameSetGrants; "(2, NULL, ''"; UCase(.tableSpaceName); "'', ?)'"
        Print #fileNo, addTab(1); ");"
        ...
        Print #fileNo, addTab(2); "CALL "; qualProcNameSetGrants; "(2, NULL, '"; UCase(.tableSpaceName); "', v_grantCount);"
        Print #fileNo, addTab(1); "END IF;"
...
Private Sub genGetSnapshotForXyzDdlV( _
  ...
  For k = 1 To tabColumns.numDescriptors
    With tabColumns.descriptors(k)
      Print #fileNo, addTab(4); "'"; .columnName; IIf(k = tabColumns.numDescriptors, "", ","); "' ||"
    End With
  Next k
  ...
  With g_classes.descriptors(classIndexSnapshot)
    Dim i As Integer
    For i = 1 To tabColumns.numDescriptors
      With tabColumns.descriptors(i)
        Print #fileNo, addTab(6); "'"; _
                                  genTransformedAttrDeclByDomain(.acmAttributeName, "???", eavtDomain, .dbDomainIndex, transformation, _
                                  eactClass, classIndexSnapshot, IIf(.isNullable, "", "NOT NULL"), False, ddlType, , , , , 0); _
    ...
    If ddlType = edtPdm Then
      If .tabSpaceData <> "" Then
        Print #fileNo, addTab(5); "' IN "; genTablespaceNameByIndex(.tabSpaceIndexData); "' ||"
      End If
      If .tabSpaceLong <> "" Then
        Print #fileNo, addTab(5); "' LONG IN "; genTablespaceNameByIndex(.tabSpaceIndexLong); "' ||"
      End If
      If .tabSpaceIndex <> "" Then
        Print #fileNo, addTab(5); "' INDEX IN "; genTablespaceNameByIndex(.tabSpaceIndexIndex); "' ||"
      End If
