...
Private Sub addXrefAttributeMappingForCl( _
  ...
  With mapping
    If .numMappings = 0 Then
      ReDim .mappings(1 To gc_allocBlockSize)
    End If
    
    Dim i As Integer
    For i = 1 To .numMappings
      With .mappings(i)
        If UCase(.mapFrom) = UCase(mapFrom) And UCase(.mapTo) = UCase(mapTo) And .isTv = isTv And .isNullable = isNullable Then
          If acmEntityType = eactClass Then
            .classIdStrList = .classIdStrList & IIf(.classIdStrList = "", "", ",") & acmEntityIdStrList
          ElseIf acmEntityType = eactRelationship Then
            .relIdStrList = .relIdStrList & IIf(.relIdStrList = "", "", ",") & acmEntityIdStrList
          End If
          Exit Sub
        End If
      End With
    ...
    If .numMappings >= UBound(.mappings) Then
      ReDim Preserve .mappings(1 To .numMappings + gc_allocBlockSize)
    End If
    .numMappings = .numMappings + 1
    With .mappings(.numMappings)
      .mapFrom = mapFrom
      .mapTo = mapTo
      .isTv = isTv
      ...
      If acmEntityType = eactClass Then
        .classIdStrList = acmEntityIdStrList
        .relIdStrList = ""
      ElseIf acmEntityType = eactRelationship Then
        .relIdStrList = acmEntityIdStrList
        .classIdStrList = ""
      End If
    End With
  End With
...
Private Sub genDataCheckUtilitiesDdlByPool( _
  ...
  With g_pools.descriptors(thisPoolIndex)
    If Not .supportLrt And (.id <> g_orgs.descriptors(thisOrgIndex).setProductiveTargetPoolId) Then
      GoTo NormalExit
    End If
  End With
...
If genDataCheckCl Then
  ...
  For i = 1 To g_attributes.numDescriptors
    With g_attributes.descriptors(i)
      If Not strArrayIsNull(.mapsToChangeLogAttributes) Then
        Dim k As Integer
        For k = LBound(.mapsToChangeLogAttributes) To UBound(.mapsToChangeLogAttributes)
          addXrefAttributeMappingForCl xrefMapping, .attributeName, .mapsToChangeLogAttributes(k), .cType, _
            IIf(.cType = eactClass, g_classes.descriptors(.acmEntityIndex).subclassIdStrListNonAbstract, g_relationships.descriptors(.acmEntityIndex).relIdStr)
        Next k
      End If
    End With
  Next i
  ...
  For i = 1 To xrefMapping.numMappings
    With xrefMapping.mappings(i)
      Print #fileNo, addTab(1); "-- CL-attribute (xref)"; .mapFrom; " maps to "; .mapTo; "["; .isTv; " / "; .isNullable; " / "; .classIdStrList; " / "; .relIdStrList; "]"
      ...
      Print #fileNo, addTab(2); "CL."; UCase(.mapTo); " AS COLUMN,"
      ...
      If .relIdStrList <> "" Then
        ...
        Print #fileNo, addTab(5); "AND"
        Print #fileNo, addTab(4); "CL."; g_anAcmEntityId; " IN ("; .relIdStrList; ")"
        Print #fileNo, addTab(3); ")"
      End If
      Print #fileNo, addTab(2); ")"
      
      Print #fileNo, addTab(3); "AND"
      Print #fileNo, addTab(2); "CL."; UCase(.mapTo); " IS NULL"
      ...
      Print #fileNo, addTab(2); "CL.OPERATION_ID,"
      Print #fileNo, addTab(2); "CL."; g_anPsOid; ","
      Print #fileNo, addTab(2); "CL."; UCase(.mapTo)
      Print #fileNo, addTab(1); ";"
      ...
      If .classIdStrList <> "" Then
        ...
        Print #fileNo, addTab(2); qualTabNameChangeLog; " CL"
        Print #fileNo, addTab(1); "WHERE"
        Print #fileNo, addTab(2); "CL."; g_anAhCid; " IN ("; .classIdStrList; ")"
        Print #fileNo, addTab(3); "AND"
        Print #fileNo, addTab(2); "CL."; UCase(.mapTo); " IS NULL"
        Print #fileNo, addTab(1); ";"
      End If
