  ...
  With mapping
    ...
    With g_classes.descriptors(classIndex)
      If .attrRefs.numDescriptors > 0 Then
        ...
        mapping.idAttrName = g_attributes.descriptors(.attrRefs.descriptors(1).refIndex).attributeName
      Else
        mapping.idAttrName = ""
      End If
    End With
...
Sub genTraceDdlByPool( _
  ...
  For i = 1 To numClasses
    With g_classes.descriptors(tabMapping(i).classIndex)
      If .specificToOrgId <= 0 Or .specificToOrgId = thisOrgId Then
        If .specificToPool < 0 Or .specificToPool = thisPoolId Then
          nothingToDo = False
        End If
      End If
    End With
  ...
  For i = 1 To numClasses
    With tabMapping(i)
      ...
      Print #fileNo, addTab(2); .tempTabName
  ...
  For i = 1 To numClasses
    With tabMapping(i)
      ...
      genAttrListForEntity .classIndex, eactClass, fileNo, ddlType, thisOrgIndex, thisPoolIndex, 2, , , edomListNonLrt
  
      Print #fileNo, addTab(1); ")"
      Print #fileNo, addTab(1); "SELECT"
  
      initAttributeTransformation transformation, 1
      setAttributeMapping transformation, 1, .idAttrName, "v_traceId"
  
      genTransformedAttrListForEntity .classIndex, eactClass, transformation, fileNo, ddlType, thisOrgIndex, thisPoolIndex, 2, , , , edomListNonLrt
