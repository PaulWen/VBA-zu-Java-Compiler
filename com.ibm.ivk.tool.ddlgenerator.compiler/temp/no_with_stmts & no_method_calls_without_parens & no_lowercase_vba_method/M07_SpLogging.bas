     ...
     ElseIf spLogMode = esplTable Then
       ...
       Else
         ...
         For i = 1 To tabColumns.numDescriptors
             Print #fileNo, addTab(5); "'"; tabColumns.descriptors(i).columnName; IIf(i < tabColumns.numDescriptors, ",", ""); "' ||"
         Next i
         Print #fileNo, addTab(4); "') VALUES (' ||"
         For i = 1 To tabColumns.numDescriptors
             If tabColumns.descriptors(i).columnName = conEventType Then
               Print #fileNo, addTab(5); "COALESCE('''' || "; g_anEventType; " || '''', 'NULL')"; IIf(i < tabColumns.numDescriptors, " || ','", ""); " ||"
             ElseIf tabColumns.descriptors(i).columnName = conSpLogContextSchema Then
               Print #fileNo, addTab(5); "COALESCE('''' || "; g_anSpLogContextSchema; " || '''', 'NULL')"; IIf(i < tabColumns.numDescriptors, " || ','", ""); " ||"
             ElseIf tabColumns.descriptors(i).columnName = conSpLogContextName Then
               Print #fileNo, addTab(5); "COALESCE('''' || "; g_anSpLogContextName; " || '''', 'NULL')"; IIf(i < tabColumns.numDescriptors, " || ','", ""); " ||"
             ElseIf tabColumns.descriptors(i).columnName = conSpLogContextType Then
