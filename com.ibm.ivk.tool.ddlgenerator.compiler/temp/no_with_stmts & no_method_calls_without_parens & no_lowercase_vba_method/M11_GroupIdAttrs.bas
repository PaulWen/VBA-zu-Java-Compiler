     ...
     For k = LBound(groupIdAttrIndexes) To UBound(groupIdAttrIndexes)
         ...
         For l = LBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes) To UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes)
           ...
           For m = 1 To tabColumns.numDescriptors
             If tabColumns.descriptors(m).columnName = UCase(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l)) Then
               If tabColumns.descriptors(m).acmAttributeIndex > 0 Then
                   If g_domains.descriptors(g_attributes.descriptors(tabColumns.descriptors(m).acmAttributeIndex).domainIndex).maxLength = "" Then
                     effectiveMaxLength = ""
                   Else
                     If supportUnicode And g_domains.descriptors(g_attributes.descriptors(tabColumns.descriptors(m).acmAttributeIndex).domainIndex).supportUnicode Then
                       effectiveMaxLength = CInt(g_domains.descriptors(g_attributes.descriptors(tabColumns.descriptors(m).acmAttributeIndex).domainIndex).unicodeExpansionFactor * CInt(g_domains.descriptors(g_attributes.descriptors(tabColumns.descriptors(m).acmAttributeIndex).domainIndex).maxLength)) & ""
                     Else
                       effectiveMaxLength = g_domains.descriptors(g_attributes.descriptors(tabColumns.descriptors(m).acmAttributeIndex).domainIndex).maxLength
                     End If
                   End If

                   genVarDecl(fileNo, varNamePrefix1 & tabColumns.descriptors(m).columnName & CStr(k), getDataType(g_domains.descriptors(g_attributes.descriptors(tabColumns.descriptors(m).acmAttributeIndex).domainIndex).dataType, effectiveMaxLength, g_domains.descriptors(g_attributes.descriptors(tabColumns.descriptors(m).acmAttributeIndex).domainIndex).scale), "NULL")
                 GoTo exitM
               End If
             End If
           Next m
           ...
           ElseIf InStr(1, UCase(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l)), "VALID") > 0 Then
             genVarDecl(fileNo, varNamePrefix1 & UCase(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l)) & CStr(k), "DATE", "NULL")
           Else
             genVarDecl(fileNo, varNamePrefix1 & UCase(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l)) & CStr(k), "BIGINT", "NULL")
           End If
 exitM:
         Next l
         ...
         For l = LBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes) To UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes)
           For m = 1 To tabColumns.numDescriptors
             If tabColumns.descriptors(m).columnName = UCase(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l)) Then
               If tabColumns.descriptors(m).acmAttributeIndex > 0 Then
                   ...
                   genVarDecl(fileNo, varNamePrefix2 & tabColumns.descriptors(m).columnName & CStr(k), getDataType(g_domains.descriptors(g_attributes.descriptors(tabColumns.descriptors(m).acmAttributeIndex).domainIndex).dataType, effectiveMaxLength, g_domains.descriptors(g_attributes.descriptors(tabColumns.descriptors(m).acmAttributeIndex).domainIndex).scale), "NULL")
                 GoTo exitMM
               End If
             End If
           Next m
           ...
           ElseIf InStr(1, UCase(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l)), "VALID") > 0 Then
             genVarDecl(fileNo, varNamePrefix2 & UCase(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l)) & CStr(k), "DATE", "NULL")
           Else
             genVarDecl(fileNo, varNamePrefix2 & UCase(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l)) & CStr(k), "BIGINT", "NULL")
           End If
 exitMM:
         Next l
     ...
     For k = LBound(groupIdAttrIndexes) To UBound(groupIdAttrIndexes)
         gidColName = genAttrName(g_attributes.descriptors(groupIdAttrIndexes(k)).attributeName, ddlType)
         gidColShortName = g_attributes.descriptors(groupIdAttrIndexes(k)).shortName
         subClassIdStrList(k) = g_classes.descriptors(g_attributes.descriptors(groupIdAttrIndexes(k)).acmEntityIndex).subclassIdStrListNonAbstract
         qualSeqNameGroupId = _
           genQualObjName( _
             getSectionIndexByName(g_attributes.descriptors(groupIdAttrIndexes(k)).sectionName), _
             "SEQ_" & entityShortName & g_attributes.descriptors(groupIdAttrIndexes(k)).shortName, _
             "SEQ_" & entityShortName & g_attributes.descriptors(groupIdAttrIndexes(k)).shortName, _
         ...
         For l = LBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes) To UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes)
           If Left(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l), 1) = "#" Then
             expGroupIdColNo = expGroupIdColNo + 1
             Print #fileNo, addTab(4); mapExpression(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l), thisOrgIndex, thisPoolIndex, ddlType); " AS EXP_"; CStr(expGroupIdColNo); IIf(l < UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes), ",", "")
           Else
             Print #fileNo, addTab(4); UCase(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l)); IIf(l < UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes), ",", "")
           End If
         Next l
         ...
         For l = LBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes) To UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes)
           If Left(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l), 1) = "#" Then
             expGroupIdColNo = expGroupIdColNo + 1
             Print #fileNo, addTab(3); "EXP_"; CStr(expGroupIdColNo); IIf(l < UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes), ",", "")
           Else
             Print #fileNo, addTab(3); UCase(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l)); IIf(l < UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes), ",", "")
           End If
         Next l
         ...
         End If
         For l = LBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes) To UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes)
           Print #fileNo, addTab(3); mapExpression(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l), thisOrgIndex, thisPoolIndex, ddlType); IIf(l < UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes), ",", "")
         Next l
         Print #fileNo, addTab(2); "FROM"
         ...
         End If
         For l = LBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes) To UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes)
           Print #fileNo, addTab(3); mapExpression(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l), thisOrgIndex, thisPoolIndex, ddlType); ","
         Next l
     ...
     For k = LBound(groupIdAttrIndexes) To UBound(groupIdAttrIndexes)
         ...
         genProcSectionHeader(fileNo, "temporary table for GroupId-OID mapping (attribute """ & UCase(g_attributes.descriptors(groupIdAttrIndexes(k)).attributeName) & """)")
     ...
     For k = LBound(groupIdAttrIndexes) To UBound(groupIdAttrIndexes)
         ...
         If thisPoolIndex <> g_workDataPoolIndex Then
           ...
           genProcSectionHeader(fileNo, "import group-ID column """ & UCase(g_attributes.descriptors(groupIdAttrIndexes(k)).attributeName) & """ in table """ & qualTabName & """ from work data pool")
         ElseIf thisOrgIndex <> g_primaryOrgIndex Then
             ...
             genProcSectionHeader(fileNo, "import group-ID column """ & UCase(g_attributes.descriptors(groupIdAttrIndexes(k)).attributeName) & """ in table """ & qualTabName & """ from factory productive data pool")
         End If
         ...
         For l = LBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes) To UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes)
           If Left(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l), 1) = "#" Then
             expGroupIdColNo = expGroupIdColNo + 1
             Print #fileNo, addTab(2); varNamePrefix2; "EXP" & "_" & CStr(k) & "_" & CStr(expGroupIdColNo); IIf(l < UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes), ",", "")
           Else
             Print #fileNo, addTab(2); varNamePrefix2; UCase(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l)) & CStr(k); IIf(l < UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes), ",", "")
           End If
         Next l
         Print #fileNo, addTab(1); ";"
         ...
         For l = LBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes) To UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes)
           If Left(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l), 1) = "#" Then
             expGroupIdColNo = expGroupIdColNo + 1
             Print #fileNo, addTab(2); varNamePrefix1; "EXP" & "_" & CStr(k) & "_" & CStr(expGroupIdColNo); IIf(l < UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes), ",", "")
           Else
             Print #fileNo, addTab(2); varNamePrefix1; UCase(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l)) & CStr(k); IIf(l < UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes), ",", "")
           End If
         Next l
         Print #fileNo, addTab(1); ";"
         ...
         For l = LBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes) To UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes)
           ...
           Else
             v1 = paddRight(varNamePrefix1 & UCase(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l)) & CStr(k), maxVarNameLength)
             v2 = paddRight(varNamePrefix2 & UCase(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l)) & CStr(k), maxVarNameLength)
           End If

           Print #fileNo, addTab(3); "((("; v1; " IS NULL) AND ("; v2; " IS NULL)) OR ("; v1; " =  "; v2; "))"; IIf(l < UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes), " AND", "")
         Next l
         ...
         For l = LBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes) To UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes)
           If Left(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l), 1) = "#" Then
             expGroupIdColNo = expGroupIdColNo + 1
             Print #fileNo, addTab(4); varNamePrefix1; "EXP"; "_"; CStr(k); "_"; CStr(expGroupIdColNo); IIf(l < UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes), ",", "")
           Else
             Print #fileNo, addTab(4); varNamePrefix1; UCase(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l)) & CStr(k); IIf(l < UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes), ",", "")
           End If
         Next l
         Print #fileNo, addTab(3); ";"
         ...
         For l = LBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes) To UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes)
           If Left(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l), 1) = "#" Then
             expGroupIdColNo = expGroupIdColNo + 1
             Print #fileNo, addTab(3); varNamePrefix2; "EXP"; "_"; CStr(k); "_"; CStr(expGroupIdColNo); IIf(l < UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes), ",", "")
           Else
             Print #fileNo, addTab(3); varNamePrefix2; UCase(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes(l)) & CStr(k); IIf(l < UBound(g_attributes.descriptors(groupIdAttrIndexes(k)).groupIdAttributes), ",", "")
           End If
         Next l
         Print #fileNo, addTab(2); ";"
