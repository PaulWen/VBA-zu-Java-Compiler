 ...
 Private Sub readSheet()
   ...
   While thisSheet.Cells(thisRow, colModuleName) & "" <> ""
     ...
     End If
       ...
       g_profLevels.descriptors(allocProfLevelDescriptorIndex(g_profLevels)).level = getInteger(thisSheet.Cells(thisRow, colLevel), 1)
       g_profLevels.descriptors(allocProfLevelDescriptorIndex(g_profLevels)).moduleName = baseName(Trim(thisSheet.Cells(thisRow, colModuleName)), ".bas")
       g_profLevels.descriptors(allocProfLevelDescriptorIndex(g_profLevels)).procName = Trim(thisSheet.Cells(thisRow, colProcName))
