 ...
 Private Sub readSheet()
   ...
   While thisSheet.Cells(thisRow, colOrg) & "" <> ""
     thisOrgId = CInt(thisSheet.Cells(thisRow, colOrg))
       ...
       g_orgs.descriptors(allocOrgIndex(g_orgs)).id = thisOrgId
       g_orgs.descriptors(allocOrgIndex(g_orgs)).name = Trim(thisSheet.Cells(thisRow, colName))
       g_orgs.descriptors(allocOrgIndex(g_orgs)).isPrimary = getBoolean(thisSheet.Cells(thisRow, colIsPrimary))
       g_orgs.descriptors(allocOrgIndex(g_orgs)).isTemplate = getBoolean(thisSheet.Cells(thisRow, colIsTemplate))
       g_orgs.descriptors(allocOrgIndex(g_orgs)).oid = getInteger(thisSheet.Cells(thisRow, colOid))
       g_orgs.descriptors(allocOrgIndex(g_orgs)).sequenceCacheSize = getInteger(thisSheet.Cells(thisRow, colSequenceCacheSize), -1)

       If Not genTemplateDdl And g_orgs.descriptors(allocOrgIndex(g_orgs)).isTemplate Then
     ...
     For i = 1 To g_attributes.numDescriptors
       If g_attributes.descriptors(i).groupIdBasedOn <> "" Then
         ...
         If g_attributes.descriptors(i).cType = eactClass Then
             If g_classes.descriptors(g_classes.descriptors(g_attributes.descriptors(i).acmEntityIndex).orMappingSuperClassIndex).specificToOrgId > 0 And thisOrgId <> g_classes.descriptors(g_classes.descriptors(g_attributes.descriptors(i).acmEntityIndex).orMappingSuperClassIndex).specificToOrgId Then
               GoTo NextI
             End If
             qualSeqNameGroupId = _
               genQualObjName( _
                 g_classes.descriptors(g_classes.descriptors(g_attributes.descriptors(i).acmEntityIndex).orMappingSuperClassIndex).sectionIndex, "SEQ_" & g_classes.descriptors(g_classes.descriptors(g_attributes.descriptors(i).acmEntityIndex).orMappingSuperClassIndex).shortName & attrShortName, "SEQ_" & g_classes.descriptors(g_classes.descriptors(g_attributes.descriptors(i).acmEntityIndex).orMappingSuperClassIndex).shortName & attrShortName, ddlType, thisOrgIndex _
               )
         Else
         ...
         If qualSeqNameGroupId <> "" And lastQualSeqNameGroupId <> qualSeqNameGroupId Then
           genSequence(_
             "Sequence for Group IDs for Column """ & g_attributes.descriptors(i).attributeName & "@" & g_attributes.descriptors(i).className & """", qualSeqNameGroupId, thisOrgIndex, _
 ...
 Sub genOidSequenceForOrg( _
   ...
   If thisOrgIndex < 1 Then
     ...
     thisOrgId = -1
   Else
       orgSeqCacheSize = g_orgs.descriptors(thisOrgIndex).sequenceCacheSize
       isCtoSequence = g_orgs.descriptors(thisOrgIndex).isPrimary And (thisOrgIndex < 1)
       orgIsTemplate = g_orgs.descriptors(thisOrgIndex).isTemplate
       thisOrgId = g_orgs.descriptors(thisOrgIndex).id
   End If
   ...
   If forOrgIndex > 0 Then
       forOrgIsTemplate = g_orgs.descriptors(forOrgIndex).isTemplate
       forOrgId = g_orgs.descriptors(forOrgIndex).id
   Else
     forOrgId = -1
   End If
   If Not forOrgIsTemplate Then
     For thisPoolIndex = 1 To g_pools.numDescriptors
         poolSeqCacheSize = g_pools.descriptors(thisPoolIndex).sequenceCacheSize
         If (((g_pools.descriptors(thisPoolIndex).specificToOrgId = -1) Or (g_pools.descriptors(thisPoolIndex).specificToOrgId = thisOrgId)) And g_pools.descriptors(thisPoolIndex).commonItemsLocal) Then
           genSequence(_
     ...
     Next thisPoolIndex
       ...
       genSequence(_
         ...
         IIf(g_orgs.descriptors(thisOrgIndex).isTemplate, genTemplateParamWrapper(g_orgs.descriptors(thisOrgIndex).name), g_orgs.descriptors(thisOrgIndex).name) & """", _
 ...
 Sub evalOrgs()
   Dim i As Integer
   For i = 1 To g_orgs.numDescriptors Step 1
       g_orgs.descriptors(i).setProductiveTargetPoolId = g_productiveDataPoolId
       g_orgs.descriptors(i).setProductiveTargetPoolIndex = getDataPoolIndexById(g_orgs.descriptors(i).setProductiveTargetPoolId)
   Next i
 End Sub
 ' ### ENDIF IVK ###
