 ...
 Private Sub genPsDeleteSupportDdlByPool( _
   ...
   Print #fileNo, addTab(1); "WHERE"
     Print #fileNo, addTab(2); genSurrogateKeyName(ddlType, g_classes.descriptors(g_classIndexProductStructure).shortName); " = psOid_in"
   Print #fileNo, addTab(1); ";"
   ...
   If Not forPsDelete Then
     ...
     If Not forNl Then
       If acmEntityType = eactClass Then
           isGenericAspect = (UCase(g_classes.descriptors(acmEntityIndex).className) = UCase(clnGenericAspect))
       End If
     End If
     ...
     For i = 1 To tabColumns.numDescriptors
         If ((tabColumns.descriptors(i).columnCategory And eacOid) Or (tabColumns.descriptors(i).columnCategory And eacCid) Or _
             (tabColumns.descriptors(i).columnCategory And eacPsOid) Or (tabColumns.descriptors(i).columnCategory And eacLrtMeta) Or _
             (tabColumns.descriptors(i).columnCategory And eacFkOidParent)) And _
            ((tabColumns.descriptors(i).columnCategory And eacRegular) = 0) Then
           ' ignore this column
         Else
             If (g_domains.descriptors(tabColumns.descriptors(i).dbDomainIndex).dataType = etBlob) Or (g_domains.descriptors(tabColumns.descriptors(i).dbDomainIndex).dataType = etClob) Then
           ...
           ElseIf isGenericAspect And (tabColumns.descriptors(i).columnCategory = eacFkOid) And _
              (Len(tabColumns.descriptors(i).columnName) = 10) And _
              ((Left(tabColumns.descriptors(i).columnName, 4) = "S0CS") Or (Left(tabColumns.descriptors(i).columnName, 4) = "N1CN") Or (Left(tabColumns.descriptors(i).columnName, 4) = "S1CT")) And _
              (Right(tabColumns.descriptors(i).columnName, 4) = "_OID") Then
             ...
             ' -> ignore this column
           Else
             colList = colList & IIf(colList = "", "", ",") & tabColumns.descriptors(i).columnName
           End If
         End If
     Next i
   End If
 ...
 Sub genTabListView( _
   ...
   For thisEntityIndex = 1 To g_classes.numDescriptors
     fkAttrToDiv = ""
       If forPsDelete Then
         processEntity = Not (g_classes.descriptors(thisEntityIndex).superClassIndex > 0) And _
         ((g_classes.descriptors(thisEntityIndex).specificToPool <= 0) Or (g_classes.descriptors(thisEntityIndex).specificToPool = g_pools.descriptors(srcPoolIndex).id)) And g_classes.descriptors(thisEntityIndex).isPsTagged
       Else
         processEntity = Not g_classes.descriptors(thisEntityIndex).isCommonToOrgs And Not g_classes.descriptors(thisEntityIndex).isCommonToPools And Not g_classes.descriptors(thisEntityIndex).isLrtSpecific And _
                         Not g_classes.descriptors(thisEntityIndex).notAcmRelated And Not (g_classes.descriptors(thisEntityIndex).superClassIndex > 0) And (g_classes.descriptors(thisEntityIndex).specificToPool <= 0)
       End If
       If processEntity Then
         If g_classes.descriptors(thisEntityIndex).isPsTagged Then
           filter = psOidFilterStr
         Else
           navPathFromClassToDiv = g_classes.descriptors(thisEntityIndex).navPathToDiv
           If navPathFromClassToDiv.relRefIndex > 0 Then
               fkAttrToDiv = IIf(navPathFromClassToDiv.navDirection = etLeft, g_relationships.descriptors(navPathFromClassToDiv.relRefIndex).leftFkColName(ddlType), g_relationships.descriptors(navPathFromClassToDiv.relRefIndex).rightFkColName(ddlType))
           End If
         ...
         Dim i As Integer
         For i = 1 To IIf(g_classes.descriptors(thisEntityIndex).isUserTransactional And forPsDelete, 2, 1)
           forLrt = (i = 2)
           ...
           If g_classes.descriptors(thisEntityIndex).hasNlAttrsInNonGenInclSubClasses Then
             parentFkAttr = genAttrName(conOid, ddlType, g_classes.descriptors(thisEntityIndex).shortName)
             qualSrcTabNameNl = genQualTabNameByClassIndex(g_classes.descriptors(thisEntityIndex).classIndex, ddlType, thisOrgIndex, srcPoolIndex, , forLrt, , True)
             qualDstTabNameNl = genQualTabNameByClassIndex(g_classes.descriptors(thisEntityIndex).classIndex, ddlType, thisOrgIndex, dstPoolIndex, , forLrt, , True)
           ...
           If g_classes.descriptors(thisEntityIndex).isGenForming And Not g_classes.descriptors(thisEntityIndex).hasNoIdentity Then
             ...
             If g_classes.descriptors(thisEntityIndex).hasNlAttrsInGenInclSubClasses Then
                 qualSrcTabNameNl = genQualTabNameByClassIndex(g_classes.descriptors(thisEntityIndex).classIndex, ddlType, thisOrgIndex, srcPoolIndex, True, forLrt, , True)
                 qualDstTabNameNl = genQualTabNameByClassIndex(g_classes.descriptors(thisEntityIndex).classIndex, ddlType, thisOrgIndex, dstPoolIndex, True, forLrt, , True)
   ...
   For thisEntityIndex = 1 To g_relationships.numDescriptors
     fkAttrToDiv = ""
       If forPsDelete Then
         processEntity = Not g_relationships.descriptors(thisEntityIndex).notAcmRelated And (g_relationships.descriptors(thisEntityIndex).specificToPool <= 0) And g_relationships.descriptors(thisEntityIndex).isPsTagged And _
                         ((g_relationships.descriptors(thisEntityIndex).maxLeftCardinality < 0 And g_relationships.descriptors(thisEntityIndex).maxRightCardinality < 0) Or g_relationships.descriptors(thisEntityIndex).isNl)
       Else
         processEntity = Not g_relationships.descriptors(thisEntityIndex).isCommonToOrgs And Not g_relationships.descriptors(thisEntityIndex).isCommonToPools And Not g_relationships.descriptors(thisEntityIndex).isLrtSpecific And _
                         Not g_relationships.descriptors(thisEntityIndex).notAcmRelated And (g_relationships.descriptors(thisEntityIndex).specificToPool <= 0) And _
                         ((g_relationships.descriptors(thisEntityIndex).maxLeftCardinality < 0 And g_relationships.descriptors(thisEntityIndex).maxRightCardinality < 0) Or g_relationships.descriptors(thisEntityIndex).isNl)
       End If
 
       If processEntity Then
         If g_relationships.descriptors(thisEntityIndex).isPsTagged Then
           filter = psOidFilterStr
         Else
           navPathFromRelToDiv = g_relationships.descriptors(thisEntityIndex).navPathToDiv
           ...
           navToFirstClassToDivDirection = g_relationships.descriptors(thisEntityIndex).navPathToDiv.navDirectionToClass
           ...
           If navToFirstClassToDivDirection = etLeft Then
             ...
             navRefClassIndex = g_relationships.descriptors(thisEntityIndex).leftEntityIndex
             navRefClassShortName = g_classes.descriptors(g_relationships.descriptors(thisEntityIndex).leftEntityIndex).shortName
             fkAttrToClass = genSurrogateKeyName(ddlType, navRefClassShortName)
               navToDivRelRefIndex = g_classes.descriptors(g_relationships.descriptors(thisEntityIndex).leftEntityIndex).navPathToDiv.relRefIndex
               navToDivDirection = g_classes.descriptors(g_relationships.descriptors(thisEntityIndex).leftEntityIndex).navPathToDiv.navDirection
           ElseIf navToFirstClassToDivDirection = etRight Then
             ...
             navRefClassIndex = g_relationships.descriptors(thisEntityIndex).rightEntityIndex
             navRefClassShortName = g_classes.descriptors(g_relationships.descriptors(thisEntityIndex).rightEntityIndex).shortName
             fkAttrToClass = genSurrogateKeyName(ddlType, navRefClassShortName)
               navToDivRelRefIndex = g_classes.descriptors(g_relationships.descriptors(thisEntityIndex).rightEntityIndex).navPathToDiv.relRefIndex
               navToDivDirection = g_classes.descriptors(g_relationships.descriptors(thisEntityIndex).rightEntityIndex).navPathToDiv.navDirection
           End If
         ...
         For i = 1 To IIf(g_relationships.descriptors(thisEntityIndex).isUserTransactional And forPsDelete, 2, 1)
           forLrt = (i = 2)
           ...
           If g_relationships.descriptors(thisEntityIndex).nlAttrRefs.numDescriptors > 0 Then
             parentFkAttr = genAttrName(conOid, ddlType, g_relationships.descriptors(thisEntityIndex).shortName)
             qualSrcTabNameNl = genQualTabNameByRelIndex(g_relationships.descriptors(thisEntityIndex).relIndex, ddlType, thisOrgIndex, srcPoolIndex, forLrt, , True)
             qualDstTabNameNl = genQualTabNameByRelIndex(g_relationships.descriptors(thisEntityIndex).relIndex, ddlType, thisOrgIndex, dstPoolIndex, forLrt, , True)
